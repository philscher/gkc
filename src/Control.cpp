...
void Control::setSignalHandler() 
{
  ...
  // Set Floating Point Capturing (FE_ALL_EXCEPT also possible
  // but may produce underflows ?)
  feenableexcept(FE_DIVBYZERO | FE_INVALID);

  int signals[] = { SIGFPE, SIGINT, SIGUSR1, SIGUSR2, SIGSEGV, SIGBUS };

  //foreach(sig in signals) :  
  //gfor(int i = 0; i < 5; i++) signal(signals[i], signal_handler);
...
bool Control::checkOK(Timing timing, Timing maxTiming) 
{
  ...
  // check Signals
  cntrl.check(!control_triggered_signal, "(3) Interrupted by Signal");
