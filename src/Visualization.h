...
class Visualization {
 ...
 bool visXV;
  ...
  public:
    Visualization(Grid *grid, Parallel *parallel, Setup *setup, FileIO *fileIO, Vlasov *_vlasov, Fields *_fields) : vlasov(_vlasov), fields(_fields) {
...
//     FA_slphiTime->create(visualGroup, "Time", offset0);
    ...
    dataOutputVisual      = Timing(setup->get("DataOutput.Visualization.Step", -1)       , setup->get("DataOutput.Visualization.Time", -1.));
  }   
