...
FFTSolver_fftw3::FFTSolver_fftw3(Setup *setup, Parallel *parallel, Geometry *geo) : FFTSolver(setup, parallel, geo, Nx*(2*Nky-2)*Nz, Nx*(2*Nky-2), Nx,  (2*Nky-2)) {
   ...
   if(parallel->Coord[DIR_V] == 0) {          // Fourier solver required in velocity space
      ...
      if(flags & FFT_X) {
         ...
         data_X_kIn  = (CComplex *) fftw_alloc_complex(2*X_numElements*NkyLD*NzLD*nfields);
         data_X_kOut = (CComplex *) fftw_alloc_complex(2*X_numElements*NkyLD*NzLD*nfields);
         
         data_X_rOut = (CComplex *) fftw_alloc_complex(2*X_numElements*NkyLD*NzLD*nfields);
         data_X_rIn  = (CComplex *) fftw_alloc_complex(2*X_numElements*NkyLD*NzLD*nfields);
         
         data_X_Transp_1 = (CComplex *) fftw_alloc_complex(2*X_numElements*NkyLD*NzLD*nfields);
         data_X_Transp_2 = (CComplex *) fftw_alloc_complex(2*X_numElements*NkyLD*NzLD*nfields);
...
//                 fftw_execute_r2r(plan_FieldTranspose_1, (double*) data_X_Transp_2, (double *) p);
             ...
             if(direction == FFT_BACKWARD)  fftw_execute_dft_c2r(plan_YBackward_Field, (fftw_complex *) in, (double   *) out); 
             else   check(-1, DMESG("No such FFT direction"));
        }
             ...
             if(direction == FFT_BACKWARD)  fftw_execute_dft_c2r(plan_YBackward_PSF, (fftw_complex *) in, (double   *) out); 
             else   check(-1, DMESG("No such FFT direction"));
        }
