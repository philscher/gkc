...
complex <float> SFL::c4_cos ( complex <float> z )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::c4_COS evaluates the cosine of a C4 argument.
//
//  Licensing:
//
...
//
//    Output, complex <float> SFL::c4_COS, the cosine of Z.
//
{
  float cs;
  complex <float> value;
...
complex <float> SFL::c4_sin ( complex <float> z )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::c4_SIN evaluates the sine of a C4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, complex <float> Z, the argument.
//
//    Output, complex <float> SFL::c4_SIN, the sine of Z.
//
{
  float sn;
  complex <float> value;
...
int SFL::i4_abs ( int i )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::i4_ABS returns the absolute value of an I4.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license.
...
//
//    Output, int SFL::i4_ABS, the absolute value of the integer.
//
{
  int value;
...
int SFL::i4_mach ( int i )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::i4_MACH returns integer machine constants.
//
//  Discussion:
//
//    Input/output unit numbers.
//
//      SFL::i4_MACH(1) = the standard input unit.
//      SFL::i4_MACH(2) = the standard output unit.
//      SFL::i4_MACH(3) = the standard punch unit.
//      SFL::i4_MACH(4) = the standard error message unit.
//
//    Words.
//
//      SFL::i4_MACH(5) = the number of bits per integer storage unit.
//      SFL::i4_MACH(6) = the number of characters per integer storage unit.
//
//    Integers.
//
...
//    where 0 <= X(1:S-1) < A.
//
//      SFL::i4_MACH(7) = A, the base.
//      SFL::i4_MACH(8) = S, the number of base A digits.
//      SFL::i4_MACH(9) = A^S-1, the largest integer.
//
...
//
//      SFL::i4_MACH(10) = B, the base.
//
//    Single precision
//
//      SFL::i4_MACH(11) = T, the number of base B digits.
//      SFL::i4_MACH(12) = EMIN, the smallest exponent E.
//      SFL::i4_MACH(13) = EMAX, the largest exponent E.
//
//    Double precision
//
//      SFL::i4_MACH(14) = T, the number of base B digits.
//      SFL::i4_MACH(15) = EMIN, the smallest exponent E.
//      SFL::i4_MACH(16) = EMAX, the largest exponent E.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    1 <= I <= 16.
//
//    Output, int SFL::i4_MACH, the value of the chosen parameter.
//
{
  int value;
...
int SFL::i4_max ( int i1, int i2 )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::i4_MAX returns the maximum of two I4's.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license.
...
//
//    Output, int SFL::i4_MAX, the larger of I1 and I2.
//
{
  int value;
...
int SFL::i4_min ( int i1, int i2 )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::i4_MIN returns the minimum of two I4's.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license.
...
//
//    Output, int SFL::i4_MIN, the smaller of I1 and I2.
//
{
  int value;
...
int SFL::i4_pow ( int i, int j )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::i4_POW returns the value of I^J.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license.
...
//
//    Output, int SFL::i4_POW, the value of I^J.
//
{
  int k;
  int value;
...
float SFL::r4_abs ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_ABS returns the absolute value of an R4.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, float SFL::r4_ABS, the absolute value of X.
//
{
  float value;
...
float SFL::r4_acos ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_ACOS evaluates the arc-cosine of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, float SFL::r4_ACOS, the arc-cosine of X.
//
{
  static float pi2 = 1.57079632679489661923;
  float value;

  value = pi2 - SFL::r4_asin ( x );

  return value;
}
//****************************************************************************80

float SFL::r4_acosh ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_ACOSH evaluates the arc-hyperbolic cosine of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_ACOSH, the arc-hyperbolic cosine of X.
//
{
  ...
  if ( xmax == 0.0E+00 )
  {
    xmax = 1.0E+00 / sqrt ( SFL::r4_mach ( 3 ) );
  }

  if ( x < 1.0E+00 )
  {
    cerr << "\n";
    cerr << "SFL::r4_ACOSH - Fatal error!\n";
    cerr << "  X < 1.\n";
    exit ( 1 );
  }
...
void SFL::r4_admp ( float x, float &ampl, float &phi )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_ADMP: modulus and phase of the derivative of the Airy function.
//
//  Description:
//
...
//
{
  ...
  if ( nan20 == 0 )
  {
    eta = 0.1E+00 * SFL::r4_mach ( 3 );
    nan20 = SFL::r4_inits ( an20cs, 16, eta );
    nan21 = SFL::r4_inits ( an21cs, 24, eta );
    nan22 = SFL::r4_inits ( an22cs, 33, eta );
    naph0 = SFL::r4_inits ( aph0cs, 15, eta );
    naph1 = SFL::r4_inits ( aph1cs, 22, eta );
    naph2 = SFL::r4_inits ( aph2cs, 32, eta );
    xsml = - pow ( 128.0E+00 / SFL::r4_mach ( 3 ), 0.3333E+00 );
  }

  if ( x <= xsml )
  {
    z = 1.0E+00;
    ampl = 0.3125E+00 + SFL::r4_csevl ( z, an20cs, nan20 );
    phi = - 0.625E+00 + SFL::r4_csevl ( z, aph0cs, naph0 );
  }
  else if ( x < - 4.0E+00 )
  {
  ...
  else if ( x < - 2.0E+00 )
  {
    ...
    ampl = 0.3125E+00 + SFL::r4_csevl ( z, an21cs, nan21 );
    phi = - 0.625E+00 + SFL::r4_csevl ( z, aph1cs, naph1 );
  }
  else if ( x <= - 1.0E+00 )
  {
    z = ( 16.0E+00 / x / x / x + 9.0E+00 ) / 7.0E+00;
    ampl = 0.3125E+00 + SFL::r4_csevl ( z, an22cs, nan22 );
    phi = - 0.625E+00 + SFL::r4_csevl ( z, aph2cs, naph2 );
  }
  else
  {
    cerr << "\n";
    cerr << "SFL::r4_ADMP - Fatal error!\n";
    cerr << "  - 1.0 < x.\n";
    exit ( 1 );
  }
...
float SFL::r4_ai ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_AI evaluates the Airy function Ai of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_AI, the Airy function Ai of X.
//
{
  ...
  if ( naif == 0 )
  {
    naif = SFL::r4_inits ( aifcs, 9, 0.1E+00 * SFL::r4_mach ( 3 ) );
    naig = SFL::r4_inits ( aigcs, 8, 0.1E+00 * SFL::r4_mach ( 3 ) );
    x3sml = SFL::r4_power ( SFL::r4_mach ( 3 ), 0.3334E+00 );
    xmax = SFL::r4_power ( - 1.5E+00 * log ( SFL::r4_mach ( 1 ) ), 0.6667E+00 );
  ...
  if ( x < - 1.0E+00 )
  {
    SFL::r4_aimp ( x, xm, theta );
    value = xm * cos ( theta );
  }
  else if ( SFL::r4_abs ( x ) <= x3sml )
  {
    z = 0.0E+00;
    value = 0.375E+00 + ( SFL::r4_csevl ( z, aifcs, naif ) 
      - x * ( 0.25E+00 + SFL::r4_csevl ( z, aigcs, naig ) ) );
  }
  else if ( x <= 1.0E+00 )
  {
    z = x * x * x;
...
float SFL::r4_aid ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_AID evaluates the derivative of the Airy function Ai of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_AID, the derivative of the Airy function Ai of X.
//
{
  ...
  if ( naif == 0 )
  {
    naif = SFL::r4_inits ( aifcs, 8, 0.1E+00 * SFL::r4_mach ( 3 ) );
    naig = SFL::r4_inits ( aigcs, 9, 0.1E+00 * SFL::r4_mach ( 3 ) );
    x3sml = SFL::r4_power ( SFL::r4_mach ( 3 ), 0.3334E+00 );
    x2sml = sqrt ( SFL::r4_mach ( 3 ) );
  }

  if ( x < - 1.0E+00 )
  {
    SFL::r4_admp ( x, xn, phi );
    value = xn * cos ( phi );
  }
  else if ( SFL::r4_abs ( x ) <= x2sml )
  {
    x2 = 0.0E+00;
    x3 = 0.0E+00;
    value = ( x2 * ( 0.125E+00 + SFL::r4_csevl ( x3, aifcs, naif ) ) -
      SFL::r4_csevl ( x3, aigcs, naig ) ) - 0.25E+00;
  }
  else if ( SFL::r4_abs ( x ) <= x3sml )
  {
    x2 = x * x;
...
float SFL::r4_aide ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_AIDE: exponentially scaled derivative, Airy function Ai of an R4 argument.
//
//  Discussion:
//
//    if X <= 0,
//      SFL::r4_AIDE ( X ) = SFL::r4_AID ( X )
//    else
//      SFL::r4_AIDE ( X ) = SFL::r4_AID ( X ) * exp ( 2/3 * X^(3/2) )
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_AIDE, the exponentially scaled derivative of 
...
//
{
  ...
  {
    eta = 0.1E+00 * SFL::r4_mach ( 3 );
    naif = SFL::r4_inits ( aifcs, 8, eta);
    naig = SFL::r4_inits ( aigcs, 9, eta);
    naip1 = SFL::r4_inits ( aip1cs, 25, eta );
    naip2 = SFL::r4_inits ( aip2cs, 15, eta );
    x2sml = sqrt ( eta );
    x3sml = SFL::r4_power ( eta, 0.3333E+00 );
    x32sml = 1.3104E+00 * x3sml * x3sml;
    xbig = SFL::r4_power ( SFL::r4_mach ( 2 ), 0.6666E+00 );
  }
  ...
  else if ( x <= 4.0E+00 )
  {
    ...
    value = ( - 0.28125E+00 
      - SFL::r4_csevl ( z, aip1cs, naip1 ) ) * sqrt ( sqrtx );
  }
  else if ( x < xbig )
  {
    sqrtx = sqrt ( x );
    ...
    value = ( - 0.28125E+00 
      - SFL::r4_csevl ( z, aip2cs, naip2 ) ) * sqrt ( sqrtx );
  }
  else
  {
    sqrtx = sqrt ( x );
...
float SFL::r4_aie ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_AIE evaluates the exponential scaled Airy function Ai of an R4 argument.
//
//  Discussion:
//
//    If X <= 0
//      SFL::r4_AIE ( X ) = SFL::r4_AI ( X )
//    else
//      SFL::r4_AIE ( X ) = SFL::r4_AI ( X ) * exp ( 2/3 X^(3/2) )
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_AIE, the Airy function Ai of X.
//
{
  static float aifcs[9] = {
   -0.03797135849666999750E+00,
  ...
  {
    eta = 0.1E+00 * SFL::r4_mach ( 3 );
    naif = SFL::r4_inits ( aifcs, 9, eta );
    naig = SFL::r4_inits ( aigcs, 8, eta );
    naip = SFL::r4_inits ( aipcs, 34, eta );
    x3sml = SFL::r4_power ( eta, 0.3333E+00 );
    x32sml = 1.3104E+00 * x3sml * x3sml;
    xbig = SFL::r4_power ( SFL::r4_mach ( 2 ), 0.6666E+00 );
  }
  ...
  else if ( x < xbig )
  {
    ...
    value = ( 0.28125E+00 
      + SFL::r4_csevl ( z, aipcs, naip ) ) / sqrt ( sqrtx );
  }
  else
  {
    sqrtx = sqrt ( x );
...
void SFL::r4_aimp ( float x, float &ampl, float &theta )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_AIMP evaluates the modulus and phase of the Airy function.
//
//  Description:
//
//    This function must only be called when X <= -1.0.
...
//
{
  ...
  {
    eta = 0.1E+00 * SFL::r4_mach ( 3 );
    nam21 = SFL::r4_inits ( am21cs, 40, eta );
    nath1 = SFL::r4_inits ( ath1cs, 36, eta );
    nam22 = SFL::r4_inits ( am22cs, 33, eta );
    nath2 = SFL::r4_inits ( ath2cs, 32, eta );
    xsml = - SFL::r4_power ( 16.0E+00 / SFL::r4_mach ( 3 ), 0.3333E+00 );
  }

  if ( x <= xsml )
  {
    z = 1.0E+00;
    ampl = 0.3125E+00 + SFL::r4_csevl ( z, am21cs, nam21 );
    theta = - 0.625E+00 + SFL::r4_csevl ( z, ath1cs, nath1 );
  }
  else if ( x < - 2.0E+00 )
  {
    z = 16.0 / x / x / x + 1.0;
  ...
  {
    z = ( 16.0E+00 / x / x / x + 9.0E+00 ) / 7.0E+00;
    ampl = 0.3125E+00 + SFL::r4_csevl ( z, am22cs, nam22 );
    theta = - 0.625E+00 + SFL::r4_csevl ( z, ath2cs, nath2 );
  }
  else
  {
    cerr << "\n";
    cerr << "SFL::r4_AIMP - Fatal error!\n";
    cerr << "  - 1.0 < X.\n";
    exit ( 1 );
  }
...
float SFL::r4_aint ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_AINT truncates an R4 argument to an integer.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_AINT, the truncated version of X.
//
{
  float value;
...
float SFL::r4_asin ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_ASIN evaluates the arc-sine of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_ASIN, the arc-sine of X.
//
{
  ...
  if ( nterms == 0 )
  {
    nterms = SFL::r4_inits ( asincs, 20, 0.1 * SFL::r4_mach ( 3 ) );
    sqeps = sqrt ( 6.0 * SFL::r4_mach ( 3 ) );
  }
  ...
  else if ( x < 1.0 )
  {
    ...
    if ( z <= 0.5 )
    {
      value = x * ( 1.0 + SFL::r4_csevl ( 4.0 * z - 1.0, asincs, nterms ) );
    }
    else
    {
      value = pi2 - sqrt ( 1.0 - z ) * ( 1.0 + 
        SFL::r4_csevl ( 3.0 - 4.0 * z, asincs, nterms ) );
    }

    if ( x < 0.0 )
    {
      value = - SFL::r4_abs ( value );
    }
    else if ( 0.0 < x )
    {
      value = + SFL::r4_abs ( value );
...
float SFL::r4_asinh ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_ASINH evaluates the arc-sine of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_ASINH, the arc-hyperbolic sine of X.
//
{
  static float aln2 = 0.69314718055994530942E+00;
  ...
  if ( nterms == 0 )
  {
    nterms = SFL::r4_inits ( asnhcs, 20, 0.1E+00 * SFL::r4_mach ( 3 ) );
    sqeps = sqrt ( SFL::r4_mach ( 3 ) );
    xmax = 1.0E+00 / sqeps;
  }
  ...
  if ( y <= 1.0E+00 )
  {
    ...
    {
      value = x * ( 1.0E+00 
        + SFL::r4_csevl ( 2.0E+00 * x * x - 1.0E+00, asnhcs, nterms ) );
    }
  }
  else
  {
...
float SFL::r4_atan ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_ATAN evaluates the arc-tangent of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_ATAN, the arc-tangent of X.
//
{
  ...
  if ( nterms == 0 )
  {
    nterms = SFL::r4_inits ( atancs, 9, 0.1E+00 * SFL::r4_mach ( 3 ) );
    sqeps = sqrt ( 6.0E+00 * SFL::r4_mach ( 3 ) );
    xbig = 1.0E+00 / SFL::r4_mach ( 3 );
  }
  ...
  else if ( y <= xbnd4 )
  {
    ...
    value = conpi8[n-1] + ( pi8[n-1] + t * ( 0.75E+00 +
      SFL::r4_csevl ( 50.0E+00 * t * t - 1.0E+00, atancs, nterms ) ) );
  }
  else
  {
    ...
    if ( y < xbig )
    {
      value = conpi8[3] + ( pi8[3] - ( 0.75E+00 +
        SFL::r4_csevl ( 50.0E+00 / y / y - 1.0E+00, atancs, 
        nterms ) ) / y );
    }
  }
...
float SFL::r4_atan2 ( float sn, float cs )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_ATAN2 evaluates the arc-tangent of two R4 arguments.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, double SFL::r4_ATAN2, the arc-tangent of the angle.
//
{
  float abscs;
  float abssn;
...
float SFL::r4_atanh ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_ATANH evaluates the arc-hyperbolic tangent of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_ATANH, the arc-hyperbolic tangent of X.
//
{
  ...
  if ( nterms == 0 )
  {
    nterms = SFL::r4_inits ( atnhcs, 15, 0.1E+00 * SFL::r4_mach ( 3 ) );
    dxrel = sqrt ( SFL::r4_mach ( 4 ) );
    sqeps = sqrt ( 3.0E+00 * SFL::r4_mach ( 3 ) );
  }
  ...
  {
    value = x * ( 1.0E+00 
      + SFL::r4_csevl ( 8.0E+00 * x * x - 1.0E+00, atnhcs, nterms ) );
  }
  else if ( y < 1.0E+00 )
  {
...
float SFL::r4_besi0 ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESI0 evaluates the Bessel function I of order 0 of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BESI0, the Bessel function I of order 0 of X.
//
{
  ...
  if ( nti0 == 0 )
  {
    nti0 = SFL::r4_inits ( bi0cs, 12, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( 4.0E+00 * SFL::r4_mach ( 3 ) );
    xmax = log ( SFL::r4_mach ( 2 ) );
  }
  ...
  {
    value = 2.75E+00 + SFL::r4_csevl ( y * y / 4.5E+00 - 1.0E+00, 
      bi0cs, nti0 );
  }
  else if ( y <= xmax )
  {
    value = exp ( y ) * SFL::r4_besi0e ( x );
  }
  else
  {
    cerr << "\n";
    cerr << "SFL::r4_BESI0 - Fatal error!\n";
...
float SFL::r4_besi0e ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESI0E evaluates the exponentially scaled Bessel function I0(X).
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BESI0E, the exponentially scaled Bessel 
//    function I0(X).
//
{
  ...
  {
    nti0 = SFL::r4_inits ( bi0cs, 12, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntai0 = SFL::r4_inits ( ai0cs, 21, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntai02 = SFL::r4_inits ( ai02cs, 22, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( 4.0E+00 * SFL::r4_mach ( 3 ) );
  }
  ...
  else if ( y <= 3.0E+00 )
  {
    value = exp ( - y ) * ( 2.75E+00 +
      SFL::r4_csevl ( y * y / 4.5E+00 - 1.0E+00, bi0cs, nti0 ) );
  }
  else if ( y <= 8.0E+00 )
  {
    value = ( 0.375E+00 + SFL::r4_csevl 
...
float SFL::r4_besi1 ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESI1 evaluates the Bessel function I of order 1 of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BESI1, the Bessel function I of order 1 of X.
//
{
  ...
  if ( nti1 == 0 )
  {
    nti1 = SFL::r4_inits ( bi1cs, 11, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xmin = 2.0E+00 * SFL::r4_mach ( 1 );
    xsml = sqrt ( 8.0E+00 * SFL::r4_mach ( 3 ) );
    xmax = log ( SFL::r4_mach ( 2 ) );
  }
  ...
  {
    value = x * ( 0.875E+00 + SFL::r4_csevl 
      ( y * y / 4.5E+00 - 1.0E+00, bi1cs, nti1 ) );
  }
  else if ( y <= xmax )
  {
    value = exp ( y ) * SFL::r4_besi1e ( x );
  }
  else
  {
    cerr << "\n";
    cerr << "SFL::r4_BESI1 - Fatal error!\n";
    cerr << "  Result overflows.\n";
    exit ( 1 );
  }
  return value;
...
float SFL::r4_besi1e ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESI1E: exponentially scaled Bessel function I of order 1 of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BESI1E, the exponentially scaled Bessel function I 
//    of order 1 of X.
//
{
  ...
  {
    nti1 = SFL::r4_inits ( bi1cs, 11, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntai1 = SFL::r4_inits ( ai1cs, 21, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntai12 = SFL::r4_inits ( ai12cs, 22, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xmin = 2.0E+00 * SFL::r4_mach ( 1 );
    xsml = sqrt ( 8.0E+00 * SFL::r4_mach ( 3 ) );
  }
  ...
  {
    value = x * ( 0.875E+00 
      + SFL::r4_csevl ( y * y / 4.5E+00 - 1.0E+00, bi1cs, nti1 ) );
    value = exp ( - y ) * value;
  }
  else if ( y <= 8.0E+00 )
  {
    value = ( 0.375E+00 + SFL::r4_csevl ( ( 48.0E+00 / y - 11.0E+00 ) / 5.0E+00, 
      ai1cs, ntai1) ) / sqrt ( y );
    if ( x < 0.0E+00 )
    {
      value = - value;
  ...
  {
    value = ( 0.375E+00 
      + SFL::r4_csevl ( 16.0E+00 / y - 1.0E+00, ai12cs, ntai12 ) ) / sqrt ( y );
    if ( x < 0.0E+00 )
    {
      value = - value;
    }
...
float SFL::r4_besj0 ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESJ0 evaluates the Bessel function J of order 0 of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BESJ0, the Bessel function J of order 0 of X.
//
{
  float ampl;
  ...
  if ( ntj0 == 0 )
  {
    ntj0 = SFL::r4_inits ( bj0cs, 13, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntm0 = SFL::r4_inits ( bm0cs, 21, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntth0 = SFL::r4_inits ( bth0cs, 24, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( 4.0E+00 * SFL::r4_mach ( 3 ) );
    xmax = 1.0E+00 / SFL::r4_mach ( 4 );
  }
  ...
  {
    value = SFL::r4_csevl ( 0.125E+00 * y * y - 1.0E+00, bj0cs, ntj0 );
  }
  else
  {
    z = 32.0E+00 / y / y - 1.0E+00;
    ampl = ( 0.75E+00 + SFL::r4_csevl ( z, bm0cs, ntm0 ) ) / sqrt ( y );
    theta = y - pi4 + SFL::r4_csevl ( z, bth0cs, ntth0 ) / y;
...
float SFL::r4_besj1 ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESJ1 evaluates the Bessel function J of order 1 of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BESJ1, the Bessel function J of order 1 of X.
//
{
  float ampl;
  ...
  if ( ntj1 == 0 )
  {
    ntj1 = SFL::r4_inits ( bj1cs, 12, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntm1 = SFL::r4_inits ( bm1cs, 21, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntth1 = SFL::r4_inits ( bth1cs, 24, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( 8.0E+00 * SFL::r4_mach ( 3 ) );
    xmin = 2.0E+00 * SFL::r4_mach ( 1 );
    xmax = 1.0E+00 / SFL::r4_mach ( 4 );
  }
  ...
  {
    value = x * ( 0.25E+00 
      + SFL::r4_csevl ( 0.125E+00 * y * y - 1.0E+00, bj1cs, ntj1 ) );
  }
  else
  {
    z = 32.0E+00 / y / y - 1.0E+00;
    ampl = ( 0.75E+00 + SFL::r4_csevl ( z, bm1cs, ntm1 ) ) / sqrt ( y );
    theta = y - 3.0E+00 * pi4 + SFL::r4_csevl ( z, bth1cs, ntth1 ) / y;
    if ( x < 0.0E+00 )
    {
      value = - ampl * cos ( theta );
    }
...
float SFL::r4_besk0 ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESK0 evaluates the Bessel function K of order 0 of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BESK0, the Bessel function K of order 0 of X.
//
{
  ...
  if ( ntk0 == 0 )
  {
    ntk0 = SFL::r4_inits ( bk0cs, 11, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( 4.0E+00 * SFL::r4_mach ( 3 ) );
    xmax = - log ( SFL::r4_mach ( 1 ) );
  ...
  {
    y = 0.0E+00;
    value = - log ( 0.5E+00 * x ) * SFL::r4_besi0 ( x ) 
      - 0.25E+00 + SFL::r4_csevl ( 0.5E+00 * y - 1.0E+00, bk0cs, ntk0 );
  }
  else if ( x <= 2.0E+00 )
  {
    y = x * x;
...
float SFL::r4_besk0e ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESK0E evaluates the exponentially scaled Bessel function K0(X).
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BESK0E, the exponentially scaled Bessel 
//    function K0(X).
//
{
  ...
  {
    ntk0 = SFL::r4_inits ( bk0cs, 11, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntak0 = SFL::r4_inits ( ak0cs, 17, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntak02 = SFL::r4_inits ( ak02cs, 14, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( 4.0E+00 * SFL::r4_mach ( 3 ) );
  }
  ...
  {
    cerr << "\n";
    cerr << "SFL::r4_BESK0E - Fatal error!\n";
    cerr << "  X <= 0.\n";
    exit ( 1 );
  }
  else if ( x <= xsml )
  {
    y = 0.0E+00;
    value = exp ( x ) * ( - log ( 0.5E+00 * x ) * SFL::r4_besi0 ( x ) - 0.25E+00 
      + SFL::r4_csevl ( 0.5E+00 * y - 1.0E+00, bk0cs, ntk0 ) );
  }
  else if ( x <= 2.0E+00 )
  {
    y = x * x;
  ...
  {
    value = ( 1.25E+00 + SFL::r4_csevl ( ( 16.0E+00 / x - 5.0E+00 ) / 3.0E+00, 
      ak0cs, ntak0 ) ) / sqrt ( x );
  }
  else
  {
    value = ( 1.25E+00 
      + SFL::r4_csevl ( 16.0E+00 / x - 1.0E+00, ak02cs, ntak02 ) ) / sqrt ( x );
  }
  return value;
}
//****************************************************************************80

float SFL::r4_besk1 ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESK1 evaluates the Bessel function K of order 1 of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BESK1, the Bessel function K of order 1 of X.
//
{
  ...
  if ( ntk1 == 0 )
  {
    ntk1 = SFL::r4_inits ( bk1cs, 11, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xmin = exp ( SFL::r4_max ( log ( SFL::r4_mach ( 1 ) ), 
      - log ( SFL::r4_mach ( 2 ) ) ) + 0.01E+00 );
    xsml = sqrt ( 4.0E+00 * SFL::r4_mach ( 3 ) );
    xmax = - log ( SFL::r4_mach ( 1 ) );
  ...
  {
    y = 0.0E+00;
    value = log ( 0.5E+00 * x ) * SFL::r4_besi1 ( x ) + ( 0.75E+00 
      + SFL::r4_csevl ( 0.5E+00 * y - 1.0E+00, bk1cs, ntk1 ) ) / x;
  }
  else if ( x <= 2.0E+00 )
  {
    y = x * x;
...
float SFL::r4_besk1e ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESK1E evaluates the exponentially scaled Bessel function K1(X).
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BESK1E, the exponentially scaled Bessel 
//    function K1(X).
//
{
  ...
  {
    ntk1 = SFL::r4_inits ( bk1cs, 11, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntak1 = SFL::r4_inits ( ak1cs, 17, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntak12 = SFL::r4_inits ( ak12cs, 14, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xmin = exp ( SFL::r4_max ( log ( SFL::r4_mach ( 1 ) ), 
      - log ( SFL::r4_mach ( 2 ) ) ) + 0.01E+00 );
    xsml = sqrt ( 4.0E+00 * SFL::r4_mach ( 3 ) );
  }
  ...
  {
    cerr << "\n";
    cerr << "SFL::r4_BESK1E = Fatal error!\n";
    cerr << "  X <= 0.\n";
    exit ( 1 );
  }
  else if ( x <= xsml )
  {
    y = 0.0E+00;
    value = exp ( x ) * ( log ( 0.5E+00 * x ) * SFL::r4_besi1 ( x ) 
      + ( 0.75E+00 
      + SFL::r4_csevl ( 0.5E+00 * y - 1.0E+00, bk1cs, ntk1 ) ) / x );
  }
  else if ( x <= 2.0E+00 )
  {
    y = x * x;
    value = exp ( x ) * ( log ( 0.5E+00 * x ) * SFL::r4_besi1 ( x ) + ( 0.75E+00 
      + SFL::r4_csevl ( 0.5E+00 * y - 1.0E+00, bk1cs, ntk1 ) ) / x );
  }
  else if ( x <= 8.0E+00 )
  {
  ...
  {
    value = ( 1.25E+00 
      + SFL::r4_csevl ( 16.E+00 / x - 1.0E+00, ak12cs, ntak12 ) ) / sqrt ( x );
  }
  return value;
}
//****************************************************************************80

float *SFL::r4_beskes ( float xnu, float x, int nin )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESKES evaluates a sequence of exponentially scaled K Bessel functions at X.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, float SFL::r4_BESKES[abs(NIN)], the exponentially scaled K Bessel functions.
//
{
  ...
  {
    cerr << "\n";
    cerr << "SFL::r4_BESKES - Fatal error!\n";
  ...
  SFL::r4_knus ( v, x, bke[0], bknu1, iswtch );
  ...
  if ( direct < 0.0E+00 )
  {
    SFL::r4_knus ( SFL::r4_abs ( xnu + vincr ), x, bke[1], bknu1, iswtch );
  }
  ...
  vend = SFL::r4_abs ( xnu + ( float ) ( nin ) ) - 1.0E+00;
...
float *SFL::r4_besks ( float xnu, float x, int nin )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESKS evaluates a sequence of K Bessel functions at X.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    If NIN > 0, successive values of NU count UP from XNU.
//
//    Output, float SFL::r4_BESKS[abs(NIN)], the K Bessel functions.
//
{
  float *bk;
  float expxi;
...
float SFL::r4_besy0 ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESY0 evaluates the Bessel function Y of order 0 of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BESY0, the Bessel function Y of order 0 of X.
//
{
  ...
  if ( nty0 == 0 )
  {
    nty0 = SFL::r4_inits ( by0cs, 13, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntm0 = SFL::r4_inits ( bm0cs, 21, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntth0 = SFL::r4_inits ( bth0cs, 24, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( 4.0E+00 * SFL::r4_mach ( 3 ) );
    xmax = 1.0E+00 / SFL::r4_mach ( 4 );
  ...
  else if ( x <= xsml )
  {
    ...
    value = twodpi * ( alnhaf + log ( x ) ) 
      * SFL::r4_besj0 ( x ) + 0.375E+00 
      + SFL::r4_csevl ( 0.125E+00 * y - 1.0E+00, by0cs, nty0 );
  }
  else if ( x <= 4.0E+00 )
  {
    y = x * x;
  ...
  else 
  {
    ...
    ampl = ( 0.75E+00 + SFL::r4_csevl ( z, bm0cs, ntm0 ) ) / sqrt ( x );
    theta = x - pi4 + SFL::r4_csevl (z, bth0cs, ntth0 ) / x;
...
float SFL::r4_besy1 ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BESY1 evaluates the Bessel function Y of order 1 of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BESY1, the Bessel function Y of order 1 of X.
//
{
  static float ampl;
  static float bm1cs[21] = {
  ...
  if ( nty1 == 0 )
  {
    nty1 = SFL::r4_inits ( by1cs, 14, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntm1 = SFL::r4_inits ( bm1cs, 21, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntth1 = SFL::r4_inits ( bth1cs, 24, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xmin = 1.571E+00 * exp ( SFL::r4_max ( log ( SFL::r4_mach ( 1 ) ), 
      - log ( SFL::r4_mach ( 2 ) ) ) + 0.01E+00 );
    xsml = sqrt ( 4.0E+00 * SFL::r4_mach ( 3 ) );
    xmax = 1.0E+00 / SFL::r4_mach ( 4 );
  }
  ...
  {
    y = 0.0E+00;
    value = twodpi * log ( 0.5E+00 * x ) * SFL::r4_besj1 ( x ) 
      + ( 0.5E+00 + SFL::r4_csevl ( 0.125E+00 * y - 1.0E+00, by1cs, 
      nty1 ) ) / x;
  }
  else if ( x <= 4.0E+00 )
  {
  ...
  {
    z = 32.0E+00 / x / x - 1.0E+00;
    ampl = ( 0.75E+00 + SFL::r4_csevl ( z, bm1cs, ntm1 ) ) / sqrt ( x );
    theta = x - 3.0E+00 * pi4 + SFL::r4_csevl ( z, bth1cs, ntth1 ) / x;
    value = ampl * sin ( theta );
  }
  return value;
}
...
float SFL::r4_beta ( float a, float b )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BETA evaluates the beta function of R4 arguments.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, float SFL::r4_BETA, the beta function of A and B.
//
{
  float alnsml = 0.0;
  float value;
  ...
  if ( xmax == 0.0E+00 )
  {
    SFL::r4_gaml ( xmin, xmax );
    alnsml = log ( SFL::r4_mach ( 1 ) );
  }

  if ( a <= 0.0E+00 || b <= 0.0E+00 )
  {
    cerr << "\n";
    cerr << "SFL::r4_BETA - Fatal error!\n";
  ...
  if ( a + b < xmax )
  {
    value = SFL::r4_gamma ( a ) * SFL::r4_gamma ( b ) / SFL::r4_gamma ( a + b );
    return value;
  }

  value = SFL::r4_lbeta ( a, b );
...
float SFL::r4_betai ( float x, float pin, float qin )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BETAI evaluates the incomplete beta ratio of R4 arguments.
//
//  Discussion:
//
...
//    0.0 < QIN.
//
//    Output, float SFL::r4_BETAI, the incomplete beta function ratio.
//
{
  ...
  if ( ( p + q ) * y / ( p + 1.0E+00 ) < eps )
  {
    ...
    xb = p * log ( SFL::r4_max ( y, sml ) ) - log ( p ) - SFL::r4_lbeta ( p, q );
...
//
  ...
  xb = p * log ( y ) - SFL::r4_lbeta ( ps, p ) - log ( p );
  ...
  else
  {
    ...
    if ( ps != 1.0E+00 )
    {
      n = ( int ) ( SFL::r4_max ( alneps / log ( y ), 4.0E+00 ) );
      for ( i = 1; i <= n; i++ )
      {
...
//
  if  ( 1.0E+00 < q )
  {
    xb = p * log ( y ) + q * log ( 1.0E+00 - y ) 
      - SFL::r4_lbeta ( p, q ) - log ( q );
    ib = ( int ) ( SFL::r4_max ( xb / alnsml, 0.0E+00 ) );
...
float SFL::r4_bi ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BI evaluates the Airy function Bi of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BI, the Airy function Bi of X.
//
{
  ...
  {
    eta = 0.1E+00 * SFL::r4_mach ( 3 );
    nbif = SFL::r4_inits ( bifcs, 9, eta );
    nbig = SFL::r4_inits ( bigcs, 8, eta );
    nbif2 = SFL::r4_inits ( bif2cs, 10, eta );
    nbig2 = SFL::r4_inits ( big2cs, 10, eta );
    x3sml = SFL::r4_power ( eta, 0.3333E+00 );
    xmax = SFL::r4_power ( 1.5E+00 * log ( SFL::r4_mach ( 2 ) ), 0.6666E+00 );
  }
  ...
  else if ( SFL::r4_abs ( x ) <= x3sml )
  {
    ...
    value = 0.625E+00 + SFL::r4_csevl ( z, bifcs, nbif ) 
      + x * ( 0.4375E+00 + SFL::r4_csevl ( z, bigcs, nbig ) );
  }
  else if ( x <= 1.0E+00 )
  {
    z = x * x * x;
  ...
  else if ( x <= 2.0E+00 )
  {
    ...
    value = 1.125E+00 + SFL::r4_csevl ( z, bif2cs, nbif2 ) 
      + x * ( 0.625E+00 + SFL::r4_csevl ( z, big2cs, nbig2 ) );
  }
  else
  {
    value = SFL::r4_bie ( x ) * exp ( 2.0E+00 * x * sqrt ( x ) / 3.0E+00 );
  }
  return value;
}
//****************************************************************************80

float SFL::r4_bid ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BID evaluates the derivative of the Airy function Bi of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BID, the derivative of the Airy function Bi of X.
//
{
  static float bif2cs[10] = {
  ...
  {
    eta = 0.1E+00 * SFL::r4_mach ( 3 );
    nbif = SFL::r4_inits ( bifcs, 8, eta );
    nbig = SFL::r4_inits ( bigcs, 9, eta );
    nbif2 = SFL::r4_inits ( bif2cs, 10, eta );
    nbig2 = SFL::r4_inits ( big2cs, 10, eta );
    x2sml = sqrt ( eta );
    x3sml = SFL::r4_power ( eta, 0.3333E+00 );
  ...
  else if ( SFL::r4_abs ( x ) <= x2sml )
  {
    x2 = 0.0E+00;
    x3 = 0.0E+00;
    value = x2 * ( SFL::r4_csevl ( x3, bifcs, nbif ) + 0.25E+00 ) 
      + SFL::r4_csevl ( x3, bigcs, nbig ) + 0.5E+00;
  }
  else if ( SFL::r4_abs ( x ) <= x3sml )
  {
    x2 = x * x;
  ...
  {
    z = ( 2.0E+00 * x * x * x - 9.0E+00 ) / 7.0E+00;
    value = x * x * ( SFL::r4_csevl ( z, bif2cs, nbif2 ) + 0.25E+00 ) 
      + SFL::r4_csevl ( z, big2cs, nbig2 ) + 0.5E+00;
  }
  else
  {
    value = SFL::r4_bide ( x )
...
float SFL::r4_bide ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BIDE: exponentially scaled derivative, Airy function Bi of an R4 argument.
//
//  Discussion:
//
//    if X < 0,
//      SFL::r4_BIDE ( X ) = SFL::r4_BID ( X )
//    else
//      SFL::r4_BIDE ( X ) = SFL::r4_BID ( X ) * exp ( - 2/3 * X**(3/2) )
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BIDE, the exponentially scaled derivative of 
...
//
{
  ...
  if ( nbif == 0 )
  {
    ...
    nbif2 = SFL::r4_inits ( bif2cs, 10, eta );
    nbig2 = SFL::r4_inits ( big2cs, 10, eta );
    nbip1 = SFL::r4_inits ( bip1cs, 24, eta );
    nbip2 = SFL::r4_inits ( bip2cs, 29, eta );
    x2sml = sqrt ( eta );
    x3sml = SFL::r4_power ( eta, 0.3333E+00 );
  ...
  else if ( x <= 2.0E+00 )
  {
    ...
    value = exp ( - 2.0E+00 * x * sqrt ( x ) / 3.0E+00 ) 
      * ( x * x * ( 0.25E+00 + SFL::r4_csevl ( z, bif2cs, nbif2 ) ) 
      + 0.5E+00 + SFL::r4_csevl ( z, big2cs, nbig2 ) );
  }
  else if ( x <= 4.0E+00 )
  {
    sqrtx = sqrt ( x );
    z = atr / ( x * sqrtx ) + btr;
    value = ( 0.625E+00 + SFL::r4_csevl ( z, bip1cs, nbip1 ) ) * sqrt ( sqrtx );
  }
  else if ( x < xbig )
  {
    sqrtx = sqrt ( x );
    z = 16.0E+00 / ( x * sqrtx ) - 1.0E+00;
    value = ( 0.625E+00 + SFL::r4_csevl ( z, bip2cs, nbip2 ) ) * sqrt ( sqrtx );
  }
  else
  {
    sqrtx = sqrt ( x );
...
float SFL::r4_bie ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BIE evaluates the exponentially scaled Airy function Bi of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_BIE, the exponentially scaled Airy function Bi of X.
//
{
  static float atr = 8.7506905708484345E+00;
  static float bif2cs[10] = {
  ...
  if ( nbif == 0 )
  {
    ...
    nbif2 = SFL::r4_inits ( bif2cs, 10, eta );
    nbig2 = SFL::r4_inits ( big2cs, 10, eta );
    nbip  = SFL::r4_inits ( bipcs, 24, eta );
    nbip2 = SFL::r4_inits ( bip2cs, 29, eta );
    x3sml = SFL::r4_power ( eta, 0.3333E+00 );
    x32sml = 1.3104E+00 * x3sml * x3sml;
    xbig = SFL::r4_power ( SFL::r4_mach ( 2 ), 0.6666E+00 );
  ...
  else if ( x <= 2.0E+00 )
  {
    z = ( 2.0E+00 * x * x * x - 9.0E+00 ) / 7.0E+00;
    value = exp ( - 2.0E+00 * x * sqrt ( x ) / 3.0E+00 ) 
      * ( 1.125E+00 + SFL::r4_csevl ( z, bif2cs, nbif2 )
      + x * ( 0.625E+00 + SFL::r4_csevl ( z, big2cs, nbig2 ) ) );
  }
  else if ( x <= 4.0E+00 )
  {
    sqrtx = sqrt ( x );
    z = atr / ( x * sqrtx ) + btr;
    value = ( 0.625E+00 + SFL::r4_csevl ( z, bipcs, nbip ) ) / sqrt ( sqrtx );
  }
  else if ( x <= xbig )
  {
    sqrtx = sqrt ( x );
...
float SFL::r4_binom ( int n, int m )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_BINOM evaluates the binomial coefficient using R4 arithmetic.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, float SFL::r4_BINOM, the binomial coefficient.
//
{
  ...
  if ( bilnmx == 0.0E+00 );
  {
    bilnmx = log ( SFL::r4_mach ( 2 ) );
    fintmx = 0.9E+00 / SFL::r4_mach ( 3 );
  }

  if ( n < 0 )
  {
    cerr << "\n";
    cerr << "SFL::r4_BINOM - Fatal error!\n";
    cerr << "  N < 0.\n";
    exit ( 1 );
  }
  ...
  if ( k <= 20 &&  ( float ) ( k ) * log ( ( float ) ( SFL::i4_max ( n, 1 ) ) ) <= bilnmx )
  {
    value = 1.0E+00;
    for ( i = 1; i <= k; i++ )
    {
  ...
  else
  {
    ...
    corr = SFL::r4_lgmc ( xn ) - SFL::r4_lgmc ( xk ) - SFL::r4_lgmc ( xnk );

    value = xk * log ( xnk / xk ) 
      - xn * SFL::r4_lnrel ( - ( xk - 1.0E+00 ) / xn )
  ...
  if ( value < fintmx )
  {
    value = SFL::r4_aint ( value + 0.5E+00 );
  }
...
float SFL::r4_cbrt ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_CBRT computes the cube root of an R4.
//
//  Discussion:
//
...
//
//    Output, float SFL::r4_CBRT, the cube root of X.
//
{
  ...
  if ( niter == 0 )
  {
    niter = ( int ) ( 1.443E+00 * log ( -0.106E+00 
      * log ( 0.1E+00 * SFL::r4_mach ( 3 ) ) ) + 1.0E+00 );
  }
  ...
  if ( x != 0.0E+00 )
  {
    ...
    value = SFL::r4_pak ( cbrt2[irem-1] * value, ixpnt );
  }
  return value;
}
//****************************************************************************80

float SFL::r4_chi ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_CHI evaluates the hyperbolic cosine integral of an R4 argument.
//
//  Discussion:
//
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_CHI, the hyperbolic cosine integral
//    evaluated at X.
//
{
  float value;

  value = 0.5E+00 * ( SFL::r4_ei ( x ) - SFL::r4_e1 ( x ) );

  return value;
}
//****************************************************************************80

float SFL::r4_chu ( float a, float b, float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_CHU evaluates the confluent hypergeometric function of R4 arguments.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_CHU, the function value.
//
{
  float a0;
  float aintb;
  ...
  if ( x == 0.0E+00 )
  {
    ...
    value = SFL::r4_gamma ( 1.0E+00 - b ) / SFL::r4_gamma ( 1.0E+00 + a - b );
    return value;
  }

  if ( SFL::r4_max ( SFL::r4_abs ( a ), 1.0E+00 ) 
    * SFL::r4_max ( SFL::r4_abs ( 1.0E+00 + a - b ), 1.0E+00 ) 
    < 0.99E+00 * SFL::r4_abs ( x ) )
  {
    value = SFL::r4_power ( x, - a ) * SFL::r4_chu_scaled ( a, b, x );
    return value;
  }
//
...
//
  if ( n < 1 )
  {
    ...
    sum = SFL::r4_poch ( 1.0E+00 + a - b, - a ) * sum;
  }
//
...
//
  else
  {
    ...
    if ( 0 <= m )
    {
      ...
      sum = SFL::r4_gamma ( b - 1.0E+00 ) * SFL::r4_gamr ( a ) 
        * SFL::r4_power ( x, ( 1.0 - ( float ) n ) ) * xtoeps * sum;
    }
  }
//
...
//
  ...
  factor = ( float ) SFL::i4_pow ( - 1, n ) * SFL::r4_gamr ( 1.0E+00 + a - b ) 
    * SFL::r4_power ( x, ( float ) istrt );
  ...
  pochai = SFL::r4_poch ( a, xi );
  gamri1 = SFL::r4_gamr ( xi + 1.0E+00 );
  gamrni = SFL::r4_gamr ( aintb + xi );
  b0 = factor * SFL::r4_poch ( a, xi - beps ) * gamrni 
    * SFL::r4_gamr ( xi + 1.0E+00 - beps );
//
//  x^(-beps) is close to 1.0, so we must be careful in evaluating
//  the differences.
//
  if ( SFL::r4_abs ( xtoeps - 1.0E+00 ) <= 0.5E+00 )
  {
    pch1ai = SFL::r4_poch1 ( a + xi, - beps );
    pch1i = SFL::r4_poch1 ( xi + 1.0E+00 - beps, beps );
    c0 = factor * pochai * gamrni * gamri1 * (
      - SFL::r4_poch1 ( b + xi, -beps ) + pch1ai 
      - pch1i + beps * pch1ai * pch1i );
//
//  xeps1 = (1.0 - x^(-beps)) / beps
//
    xeps1 = alnx * SFL::r4_exprel ( - beps * alnx );
    ...
    for ( i = 1; i <= 1000; i++ )
    {
      ...
      t = c0 + xeps1 * b0;
      value = value + t;
      if ( SFL::r4_abs ( t ) < eps * SFL::r4_abs ( value ) )
      {
        return value;
      }
    }
...
//
  a0 = factor * pochai * SFL::r4_gamr ( b + xi ) * gamri1 / beps;
  b0 = xtoeps * b0 / beps;
...
float SFL::r4_chu_scaled ( float a, float b, float z )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_CHU_SCALED: scaled confluent hypergeometric function of R4 arguments.
//
//  Discussion:
//
...
//
//    Output, float SFL::r4_CHU_SCALED, the function value.
//
{
  float aa[4];
  float ab;
  ...
  if ( eps == 0.0E+00 )
  {
    eps = 4.0E+00 * SFL::r4_mach ( 4 );
    sqeps = sqrt ( SFL::r4_mach ( 4 ) );
  }
  ...
  for ( i = 4; i <= 300; i++ )
  {
    ...
    if ( SFL::r4_abs ( value - aa[0] / bb[0] ) < eps * SFL::r4_abs ( value ) )
    {
      return value;
    }
//
...
//
  ...
  cerr << "\n";
  cerr << "SFL::r4_CHU_SCALED - Fatal error!\n";
...
float SFL::r4_ci ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_CI evaluates the cosine integral Ci of an R4 argument.
//
//  Discussion:
//
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_CI, the cosine integral Ci evaluated at X.
//
{
  ...
  if ( nci == 0 )
  {
    nci = SFL::r4_inits ( cics, 13, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( SFL::r4_mach ( 3 ) );
  }

  if ( x <= 0.0E+00 )
  { 
    cerr << "\n";
    cerr << "SFL::r4_CI - Fatal error!\n";
    cerr << "  X <= 0.0.\n";
    exit ( 1 );
  }
  else if ( x <= xsml )
  {
    y = - 1.0E+00;
    value = log ( x ) - 0.5E+00 + SFL::r4_csevl ( y, cics, nci );
  }
  else if ( x <= 4.0E+00 )
  {
...
float SFL::r4_cin ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_CIN evaluates the alternate cosine integral Cin of an R4 argument.
//
//  Discussion:
//
//    CIN(X) = gamma + log(X) 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_CIN, the cosine integral Cin evaluated at X.
//
{
  float absx;
  ...
  if ( ncin == 0 )
  {
    ncin = SFL::r4_inits ( cincs, 11, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xmin = sqrt ( SFL::r4_mach ( 1 ) );
  }
  ...
  {
    value = x * x * SFL::r4_csevl ( ( x * x - 8.0E+00 ) * 0.125E+00, cincs, ncin );
  }
  else
  {
    SFL::r4_sifg ( x, f, g );
...
float SFL::r4_cinh ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_CINH evaluates the alternate hyperbolic cosine integral Cinh of an R4 argument.
//
//  Discussion:
//
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_CINH, the hyperbolic cosine integral Cinh
//    evaluated at X.
//
{
  float absx;
  ...
  if ( ncinh == 0 )
  {
    ncinh = SFL::r4_inits ( cinhcs, 10, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( SFL::r4_mach ( 3 ) );
    xmin = 2.0E+00 * sqrt ( SFL::r4_mach ( 1 ) );
  }
  ...
  {
    y = - 1.0E+00;
    value = x * x * ( 0.25E+00 + SFL::r4_csevl ( y, cinhcs, ncinh ) );
  }
  else if ( x <= 3.0E+00 )
  {
    y = x * x / 4.5E+00 - 1.0E+00;
  ...
  {
    value = SFL::r4_chi ( absx ) - eul - log ( absx );
  }
  return value;
}
//****************************************************************************80

float SFL::r4_cos ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_COS evaluates the cosine of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_COS, the cosine of X.
//
{
  float absx;
  float f;
  ...
  if ( ntsn == 0 )
  {
    ntsn = SFL::r4_inits ( sincs, 10, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( 2.0E+00 * SFL::r4_mach ( 3 ) );
    xmax = 1.0E+00 / SFL::r4_mach ( 4 );
    xwarn = sqrt ( xmax );
  }
  ...
  xn = SFL::r4_aint ( y * pirec + 0.5E+00 );
  n2 = ( int ) ( SFL::r4_mod ( xn, 2.0E+00 ) + 0.5E+00 );
  xn = xn - 0.5E+00;
  f = ( absx - xn * pihi ) - xn * pilo;

  xn = 2.0E+00 * ( f * pi2rec ) * ( f * pi2rec ) - 1.0E+00;
  value = f + f * SFL::r4_csevl ( xn, sincs, ntsn );
...
float SFL::r4_cos_deg ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_COS_DEG evaluates the cosine of an R4 argument in degrees.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, float SFL::r4_COS_DEG, the cosine of X.
//
{
  int n;
  ...
  if ( SFL::r4_mod ( x, 90.0E+00 ) == 0.0E+00 )
  {
    n = ( int ) ( SFL::r4_abs ( x ) / 90.0E+00 + 0.5E+00 );
    n = ( n % 2 );
...
float SFL::r4_cosh ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_COSH evaluates the hyperbolic cosine of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_COSH, the hyperbolic cosine of X.
//
{
  float value;
  float y;
  ...
  if ( ymax == 0.0E+00 )
  {
    ymax = 1.0E+00 / sqrt ( SFL::r4_mach ( 3 ) );
  }
...
float SFL::r4_cot ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_COT evaluates the cotangent of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_COT, the cotangent of X.
//
{
  float ainty;
  float ainty2;
  ...
  if ( nterms == 0 )
  {
    nterms = SFL::r4_inits ( cotcs, 8, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xmax = 1.0E+00 / SFL::r4_mach ( 4 );
    xsml = sqrt ( 3.0E+00 * SFL::r4_mach ( 3 ) );
    xmin = exp ( SFL::r4_max ( log ( SFL::r4_mach ( 1 ) ), 
      - log ( SFL::r4_mach ( 2 ) ) ) + 0.01E+00 );
    sqeps = sqrt ( SFL::r4_mach ( 4 ) );
  }
...
//
  ...
  ainty = SFL::r4_aint ( prodbg );
  y = ( prodbg - ainty ) + 0.625E+00 * yrem + y * pi2rec;
  ainty2 = SFL::r4_aint ( y );
  ainty = ainty + ainty2;
  y = y - ainty2;

  ifn = ( int ) SFL::r4_mod ( ainty, 2.0E+00 );
  ...
  if ( 0.5E+00 < SFL::r4_abs ( x ) && y < SFL::r4_abs ( x ) * sqeps )
  {
    cerr << "\n";
    cerr << "SFL::r4_COT - Warning!\n";
  ...
  {
    value = ( 0.5E+00 
      + SFL::r4_csevl ( 32.0E+00 * y * y - 1.0E+00, cotcs, nterms ) ) / y;
  }
  else if ( y <= 0.5E+00 )
  {
    value = ( 0.5E+00 + SFL::r4_csevl ( 8.0E+00 * y * y - 1.0E+00, 
  ...
  {
    value = ( 0.5E+00 + SFL::r4_csevl ( 2.0E+00 * y * y - 1.0E+00, 
...
//****************************************************************************80
 ...
 float SFL::r4_csevl ( float x, float a[], int n )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_CSEVL evaluates a Chebyshev series.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, float SFL::r4_CSEVL, the Chebyshev series evaluated at X.
//
{
  float b0;
  float b1;
...
float SFL::r4_dawson ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_DAWSON evaluates Dawson's integral of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_DAWSON, the value of Dawson's integral at X.
//
{
  ...
  {
    eps = SFL::r4_mach ( 3 );
    ntdaw  = SFL::r4_inits ( dawcs,  13, 0.1E+00 * eps );
    ntdaw2 = SFL::r4_inits ( daw2cs, 29, 0.1E+00 * eps );
    ntdawa = SFL::r4_inits ( dawacs, 26, 0.1E+00 * eps );

    xsml = sqrt ( 1.5E+00 * eps );
    xbig = sqrt ( 0.5E+00 / eps );
    xmax = exp ( SFL::r4_min ( - log ( 2.0E+00 * SFL::r4_mach ( 1 ) ), 
      log ( SFL::r4_mach ( 2 ) ) ) - 0.01E+00 );
  }
  ...
  {
    value = x * ( 0.75E+00 
      + SFL::r4_csevl ( 2.0E+00 * y * y - 1.0E+00, dawcs, ntdaw ) );
  }
  else if ( y <= 4.0E+00 )
  {
    value = x * ( 0.25E+00 
      + SFL::r4_csevl ( 0.125E+00 * y * y - 1.0E+00, daw2cs, ntdaw2 ) );
  }
  else if ( y < xbig )
  {
    value = ( 0.5E+00 
      + SFL::r4_csevl ( 32.0E+00 / y / y - 1.0E+00, dawacs, ntdawa ) ) / x;
  }
  else if ( y <= xmax )
  {
    value = 0.5E+00 / x;
...
float SFL::r4_e1 ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_E1 evaluates the exponential integral E1 for an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_E1, the exponential integral E1 evaluated at X.
//
{
  ...
  {
    eta = 0.1E+00 * SFL::r4_mach ( 3 );
    ntae11 = SFL::r4_inits ( ae11cs, 39, eta );
    ntae12 = SFL::r4_inits ( ae12cs, 25, eta );
    nte11 = SFL::r4_inits ( e11cs, 19, eta );
    nte12 = SFL::r4_inits ( e12cs, 16, eta );
    ntae13 = SFL::r4_inits ( ae13cs, 25, eta );
    ntae14 = SFL::r4_inits ( ae14cs, 26, eta );
    xmax = - log ( SFL::r4_mach ( 1 ) );
    xmax = xmax - log ( xmax );
  }
  ...
  if ( x <= - 10.0E+00 )
  {
    value = exp ( - x ) / x * ( 1.0E+00 
      + SFL::r4_csevl ( 20.0E+00 / x + 1.0E+00, ae11cs, ntae11 ) );
  }
  else if ( x <= - 4.0E+00 )
  {
    value = exp ( - x ) / x * ( 1.0E+00 + SFL::r4_csevl ( 
      ( 40.0E+00 / x + 7.0E+00 ) / 3.0E+00, ae12cs, ntae12 ) );
  }
  else if ( x <= - 1.0E+00 )
  {
    value = - log ( SFL::r4_abs ( x ) ) + SFL::r4_csevl ( 
      ( 2.0E+00 * x + 5.0E+00 ) / 3.0E+00, e11cs, nte11 );
  }
  else if ( x == 0.0E+00 )
  {
    cerr << "\n";
    cerr << "SFL::r4_E1 - Fatal error!\n";
    cerr << "  X is zero.\n";
    exit ( 1 );
  }
  else if ( x <= 1.0E+00 )
  {
    value = ( - log ( SFL::r4_abs ( x ) ) - 0.6875E+00 + x ) 
      + SFL::r4_csevl ( x, e12cs, nte12 );
  }
  else if ( x <= 4.0E+00 )
  {
    value = exp ( - x ) / x * ( 1.0E+00 + SFL::r4_csevl ( 
...
float SFL::r4_ei ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_EI evaluates the exponential integral Ei for an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_EI, the exponential integral Ei evaluated at X.
//
{
  float value;
...
float SFL::r4_erf ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_ERF evaluates the error function of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_ERF, the error function of X.
//
{
  ...
  if ( nterf == 0 )
  {
    nterf = SFL::r4_inits ( erfcs, 13, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xbig = sqrt ( - log ( sqrtpi * SFL::r4_mach ( 3 ) ) );
    sqeps = sqrt ( 2.0E+00 * SFL::r4_mach ( 3 ) );
  }
  ...
  {
    value = x * ( 1.0E+00 
      + SFL::r4_csevl ( 2.0E+00 * x * x - 1.0E+00, erfcs, nterf ) );
  }
  else if ( y <= xbig )
  {
    value = 1.0E+00 - SFL::r4_erfc ( y );
    if ( x < 0.0E+00 )
    {
      value = - value;
    }
...
float SFL::r4_erfc ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_ERFC evaluates the co-error function of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_ERFC, the co-error function of X.
//
{
  ...
  {
    eta = 0.1E+00 * SFL::r4_mach ( 3 );
    nterf = SFL::r4_inits ( erfcs, 13, eta );
    nterfc = SFL::r4_inits ( erfccs, 24, eta );
    nterc2 = SFL::r4_inits ( erc2cs, 23, eta );

    xsml = - sqrt ( - log ( sqrtpi * SFL::r4_mach ( 3 ) ) );
    xmax = sqrt ( - log ( sqrtpi * SFL::r4_mach ( 1 ) ) );
  ...
  if ( y <= 4.0E+00 )
  {
    value = exp ( - y ) / SFL::r4_abs ( x ) * ( 0.5E+00 
      + SFL::r4_csevl ( ( 8.0E+00 / y - 5.0E+00 ) / 3.0E+00, erc2cs, 
      nterc2 ) );
  }
  else
  {
    value = exp ( - y ) / SFL::r4_abs ( x ) * ( 0.5E+00 
      + SFL::r4_csevl ( 8.0E+00 / y - 1.0E+00, erfccs, nterfc ) );
  }
...
float SFL::r4_exp ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_EXP evaluates the exponential of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_EXP, the exponential of X.
//
{
  ...
  if ( nterms == 0 )
  {
    nterms = SFL::r4_inits ( expcs, 8, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xmin = log ( SFL::r4_mach ( 1 ) ) + 0.01E+00;
    xmax = log ( SFL::r4_mach ( 2 ) ) - 0.001E+00;
  }

  if ( x < xmin )
  {
    cerr << "\n";
    cerr << "SFL::r4_EXP - Warning!\n";
  ...
  else if ( x <= xmax )
  {
    ...
    value = 1.0E+00 + ( twon16[ndx-1] + f * ( 1.0E+00 + twon16[ndx-1] ) 
      * SFL::r4_csevl ( f, expcs, nterms ) );

    value = SFL::r4_pak ( value, n16 );
  }
  else
  {
    cerr << "\n";
    cerr << "SFL::r4_EXP - Fatal error!\n";
...
float SFL::r4_exprel ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_EXPREL evaluates the exponential relative error term of an R4 argument.
//
//  Discussion:
//
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_EXPREL, the exponential relative error term
//    at X.
//
{
  float absx;
...
float SFL::r4_fac ( int n )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_FAC evaluates the factorial of an I4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, float SFL::r4_FAC, the factorial of N.
//
{
...
float SFL::r4_gami ( float a, float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_GAMI evaluates the incomplete gamma function for an R4 argument.
//
//  Discussion:
//
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_GAMI, the value of the incomplete gamma function.
//
{
  float factor;
  float value;
  ...
  {
    factor = exp ( SFL::r4_lngam ( a ) + a * log ( x ) );

    value = factor * SFL::r4_gamit ( a, x );
  }
  return value;
}
//****************************************************************************80

float SFL::r4_gamic ( float a, float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_GAMIC evaluates the complementary incomplete gamma function.
//
//  Discussion:
//
...
//    Input, float X, the evaluation point.
//
//    Output, float SFL::r4_GAMIC, the value of the incomplete gamma function.
//
{
  float aeps;
  float algap1;
  ...
  if ( eps == 0.0E+00 )
  {
    eps = 0.5E+00 * SFL::r4_mach ( 3 );
    sqeps = sqrt ( SFL::r4_mach ( 4 ) );
    alneps = - log ( SFL::r4_mach ( 3 ) );
    bot = log ( SFL::r4_mach ( 1 ) );
  }

  if ( x < 0.0E+00 )
  {
    cerr << "\n";
    cerr << "SFL::r4_GAMIC - Fatal error!\n";
    cerr << "  X < 0.\n";
    exit ( 1 );
  }
  ...
  if ( x == 0.0E+00 )
  {
    ...
    value = exp ( SFL::r4_lngam ( a + 1.0E+00 ) - log ( a ) );
    return value;
  }
  ...
  if ( x < 1.0E+00 )
  {
    if ( a <= 0.5E+00 && SFL::r4_abs ( aeps ) <= 0.001E+00 )
    {
      fm = - ( float ) ( ma );
      ...
      e = e - alx * SFL::r4_power ( x, - 0.001E+00 );

      if ( e * SFL::r4_abs ( aeps ) <= eps )
      {
        value = SFL::r4_gmic ( a, x, alx );
        return value;
      }
    }

    SFL::r4_lgams ( a + 1.0E+00, algap1, sgngam );
    gstar = SFL::r4_gmit ( a, x, algap1, sgngam, alx );
    ...
    {
      alngs = log ( SFL::r4_abs ( gstar ) );
      sgngs = SFL::r4_sign ( gstar );
    }
  }
  else
  {
    if ( a < x )
    {
      value = exp ( SFL::r4_lgic ( a, x, alx ) );
      return value;
    }
    sgngam = 1.0E+00;
    algap1 = SFL::r4_lngam ( a + 1.0E+00 );
    sgngs = 1.0E+00;
    alngs = SFL::r4_lgit ( a, x, algap1 );
  }
  ...
  sgng = SFL::r4_sign ( h ) * sga * sgngam;
  t = log ( SFL::r4_abs ( h ) ) + algap1 - log ( SFL::r4_abs ( a ) );
  value = sgng * exp ( t );
  return value;
}
//****************************************************************************80

float SFL::r4_gamit ( float a, float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_GAMIT evaluates Tricomi's incomplete gamma function for an R4 argument.
//
//  Discussion:
//
//      GAMIT = x^(-a) / gamma(a) 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_GAMIT, the function value.
//
{
  float aeps;
  float ainta;
  ...
  ainta = SFL::r4_aint ( a + 0.5E+00 * sga );
  aeps = a - ainta;
  ...
  if ( x <= 1.0E+00 )
  {
    ...
    value = SFL::r4_gmit ( a, x, algap1, sgngam, alx );
    return value;
  }

  if ( x <= a )
  {
    t = SFL::r4_lgit ( a, x, SFL::r4_lngam ( a + 1.0E+00 ) );
    value = exp ( t );
    return value;
  }

  alng = SFL::r4_lgic ( a, x, alx );
//
...
//
  ...
  {
    SFL::r4_lgams ( a + 1.0E+00, algap1, sgngam );
    t = log ( SFL::r4_abs ( a ) ) + alng - algap1;
...
void SFL::r4_gaml ( float &xmin, float &xmax )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_GAML evaluates bounds for an R4 argument of the gamma function.
//
//  Discussion:
//
...
//
{
  ...
  for ( i = 1; i <= 10; i++ )
  {
    ...
    if ( SFL::r4_abs ( xmin - xold ) < 0.005E+00 )
    {
      xmin = - xmin + 0.01E+00;
      ...
      for ( j = 1; j <= 10; j++ )
      {
        ...
        if ( SFL::r4_abs ( xmax - xold ) < 0.005E+00 )
        {
          xmax = xmax - 0.01E+00;
          xmin = SFL::r4_max ( xmin, - xmax + 1.0E+00 );
          return;
        }
      }
      cerr << "\n";
      cerr << "SFL::r4_GAML - Fatal error!\n";
...
float SFL::r4_gamma ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_GAMMA evaluates the gamma function of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_GAMMA, the gamma function of X.
//
{
  static float dxrel = 0.0;
  static float gcs[23] = {
  ...
  if ( ngcs == 0 )
  {
    ngcs = SFL::r4_inits ( gcs, 23, 0.1E+00 * SFL::r4_mach ( 3 ) );
    SFL::r4_gaml ( xmin, xmax );
    xsml = exp ( SFL::r4_max ( log ( SFL::r4_mach ( 1 ) ),
      - log ( SFL::r4_mach ( 2 ) ) ) + 0.01E+00 );
    dxrel = sqrt ( SFL::r4_mach ( 4 ) );
  }
  ...
  if ( y <= 10.0E+00 )
  {
    ...
    y = x - ( float ) ( n );
    n = n - 1;
    value = 0.9375E+00 + SFL::r4_csevl ( 2.0E+00 * y - 1.0E+00, gcs, ngcs );
    ...
    else if ( n < 0 )
    {
      ...
      if ( x < - 0.5E+00 && SFL::r4_abs ( ( x - SFL::r4_aint ( x - 0.5E+00 ) ) / x ) < dxrel )
      {
        cerr << "\n";
        cerr << "SFL::r4_GAMMA - Warning!\n";
...
//
    ...
    value = exp ( ( y - 0.5E+00 ) * log ( y ) - y + sq2pil + SFL::r4_lgmc ( y ) );
    ...
    if ( SFL::r4_abs ( ( x - SFL::r4_aint ( x - 0.5E+00 ) ) / x ) < dxrel )
    {
      cerr << "\n";
      cerr << "SFL::r4_GAMMA - Warning!\n";
      cerr << "  X too near a negative integer,\n";
...
float SFL::r4_gamr ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_GAMR evaluates the reciprocal gamma function of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_GAMR, the value of the reciprocal gamma
//    function at X.
//
{
  float alngx;
  ...
  if ( x <= 0.0E+00 && SFL::r4_aint ( x ) == x )
  {
    value = 0.0E+00;
  }
  else if ( SFL::r4_abs ( x ) <= 10.0E+00 )
  {
    value = 1.0E+00 / SFL::r4_gamma ( x );
  }
  else
  {
    SFL::r4_lgams ( x, alngx, sgngx );
...
float SFL::r4_gmic ( float a, float x, float alx )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_GMIC: complementary incomplete gamma, small X, A near negative integer.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, float SFL::r4_GMIC, the complementary incomplete 
//    gamma function.
//
{
  float alng;
  ...
  for ( k = 1; k <= mm1; k++ )
  {
    ...
    t = te / ( fm - fk );
    s = s + t;
    if ( SFL::r4_abs ( t ) < eps * SFL::r4_abs ( s ) )
    {
      break;
    }
  }
  ...
  alng = log ( value ) - SFL::r4_lngam ( fm + 1.0E+00 );
  ...
  if ( s != 0.0E+00 )
  {
    value = value + SFL::r4_sign ( s ) 
      * exp ( - fm * alx + log ( SFL::r4_abs ( s ) / fm ) );
  }
...
float SFL::r4_gmit ( float a, float x, float algap1, float sgngam, float alx )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_GMIT: Tricomi's incomplete gamma function for small X.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float ALX, the logarithm of X.
//
//    Output, float SFL::r4_GMIT, the Tricomi incomplete gamma function.
//
{
  float ae;
  float aeps;
  ...
  algs = - SFL::r4_lngam ( 1.0E+00 + aeps ) + log ( s );
  s = 1.0E+00;
  m = - ma - 1;
  t = 1.0E+00;
  ...
  sgng2 = sgngam * SFL::r4_sign ( s );
  alg2 = - x - algap1 + log ( SFL::r4_abs ( s ) );
...
int SFL::r4_inits ( float dos[], int nos, float eta )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_INITS initializes a Chebyshev series.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, int SFL::r4_INITS, the number of terms of the series needed
...
float SFL::r4_int ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_INT returns the integer part of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_INT, the integer part of X.
//
{
  int i;
  int ibase;
  ...
  if ( npart == 0 )
  {
    ibase = SFL::i4_mach ( 10 );
    xmax = 1.0E+00 / SFL::r4_mach ( 4 );
    xbig = SFL::r4_min ( ( float ) ( SFL::i4_mach ( 9 ) ), xmax );
    iscale = SFL::i4_pow ( ibase, ( int ) ( log ( xbig ) 
...
void SFL::r4_knus ( float xnu, float x, float &bknu, float &bknu1, int &iswtch )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_KNUS computes a sequence of K Bessel functions.
//
//  Discussion:
//
...
//
{
  ...
  if ( ntc0k == 0 )
  {
    ntc0k = SFL::r4_inits ( c0kcs, 16, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntznu1 = SFL::r4_inits ( znu1cs, 12, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xnusml = sqrt ( SFL::r4_mach ( 3 ) / 8.0E+00 );
    xsml = 0.1E+00 * SFL::r4_mach ( 3 );
    alnsml = log ( SFL::r4_mach ( 1 ) );
    alnbig = log ( SFL::r4_mach ( 2 ) );
    alneps = log ( 0.1E+00 * SFL::r4_mach ( 3 ) );
  }

  if ( xnu < 0.0E+00 || 1.0E+00 <= xnu )
  {
    cerr << "\n";
    cerr << "SFL::r4_KNUS - Fatal error!\n";
    cerr << "  XNU < 0 or. 1 <= XNU.\n";
    exit ( 1 );
  }
...
//
    ...
    a0 = 0.5E+00 * SFL::r4_gamma ( 1.0E+00 + v );
    b0 = 0.5E+00 * SFL::r4_gamma ( 1.0E+00 - v );
    c0 = - euler;

    if ( 0.5E+00 < ztov && xnusml < v )
    {
      c0 = - 0.75E+00 + SFL::r4_csevl ( ( 8.0E+00 * v ) * v - 1.0E+00, c0kcs, ntc0k );
    }
    ...
    else
    {
      alpha[0] = c0 - alnz * ( 0.75E+00 +
        SFL::r4_csevl ( vlnz / 0.35E+00 + 1.0E+00, znu1cs, ntznu1 ) ) * b0;
    }
    ...
    nterms = ( int ) SFL::r4_max ( 2.0E+00, 11.0E+00 
...
//
  else
  {
    ...
    nterms = SFL::i4_min ( 15, ( int ) SFL::r4_max ( 3.0E+00, an + bn * alneps ) );
    ...
    for ( inu = 1; inu <= 2; inu++ )
    {
      ...
      {
        xmu = 4.0E+00 * pow ( SFL::r4_abs ( xnu ) + 1.0E+00, 2 );
      }
...
float SFL::r4_lbeta ( float a, float b )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_LBETA evaluates the logarithm of the beta function of R4 arguments.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float A, B, the arguments.
//
//    Output, float SFL::r4_LBETA, the logarithm of the beta function of A
//    and B.
//
{
  float corr;
  ...
  {
    cerr << "\n";
    cerr << "SFL::r4_LBETA - Fatal error!\n";
    cerr << "  Both arguments must be greater than 0.\n";
    exit ( 1 );
  }
  else if ( p < 10.0E+00 && q <= 10.0E+00 )
  {
    value = log ( SFL::r4_gamma ( p ) * ( SFL::r4_gamma ( q ) / SFL::r4_gamma ( p + q ) ) );
  }
  else if ( p < 10.0E+00 )
  {
    corr = SFL::r4_lgmc ( q ) - SFL::r4_lgmc ( p + q );

    value = SFL::r4_lngam ( p ) + corr + p - p * log ( p + q ) +
      ( q - 0.5E+00 ) * SFL::r4_lnrel ( - p / ( p + q ) );
  }
  else
  {
    corr = SFL::r4_lgmc ( p ) + SFL::r4_lgmc ( q ) - SFL::r4_lgmc ( p + q );

    value = - 0.5E+00 * log ( q ) + sq2pil + corr 
      + ( p - 0.5E+00 ) * log ( p / ( p + q ) ) 
      + q * SFL::r4_lnrel ( - p / ( p + q ) );
  }
  return value;
}
//****************************************************************************80

void SFL::r4_lgams ( float x, float &algam, float &sgngam )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_LGAMS evaluates the log of |gamma(x)| and sign, for an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
{
  ...
  algam = SFL::r4_lngam ( x );
  sgngam = 1.0E+00;

  if ( x <= 0.0E+00 )
  {
    k = ( int ) ( SFL::r4_mod ( - SFL::r4_aint ( x ), 2.0E+00 ) + 0.1E+00 );
...
float SFL::r4_lgic ( float a, float x, float alx )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_LGIC evaluates the log complementary incomplete gamma function for large X.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float ALX, the logarithm of X.
//
//    Output, float SFL::r4_LGIC, the log complementary incomplete gamma function.
//
{
  static float eps = 0.0;
  float fk;
...
float SFL::r4_lgit ( float a, float x, float algap1 )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_LGIT evaluates the log of Tricomi's incomplete gamma function.
//
//  Discussion:
//
...
//
//    Output, float SFL::r4_LGIT, the log of Tricomi's incomplete
//    gamma function.
//
{
  float a1x;
...
float SFL::r4_lgmc ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_LGMC evaluates the log gamma correction factor for an R4 argument.
//
//  Discussion:
//
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_LGMC, the correction factor.
//
{
  ...
  if ( nalgm == 0 )
  {
    nalgm = SFL::r4_inits ( algmcs, 6, SFL::r4_mach ( 3 ) );
    xbig = 1.0E+00 / sqrt ( SFL::r4_mach ( 3 ) );
    xmax = exp ( SFL::r4_min ( log ( SFL::r4_mach ( 2 ) / 12.0E+00 ), 
      - log ( 12.0E+00 * SFL::r4_mach ( 1 ) ) ) );
  }

  if ( x < 10.0E+00 )
  {
    cerr << "\n";
    cerr << "SFL::r4_LGMC - Fatal error!\n";
    cerr << "  X must be at least 10.\n";
    exit ( 1 );
  }
  else if ( x < xbig )
  {
    value = SFL::r4_csevl ( 2.0E+00 * ( 10.0E+00 / x ) 
...
float SFL::r4_li ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_LI evaluates the logarithmic integral for an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_LI, the logarithmic integral evaluated at X.
//
{
  static float sqeps = 0.0;
  float value;
  ...
  if ( SFL::r4_abs ( 1.0E+00 - x ) < sqeps )
  {
    cerr << "\n";
    cerr << "SFL::r4_LI - Warning!\n";
    cerr << "  Answer less than half precision.\n";
    cerr << "  X is too close to 1.\n";
  }

  value = SFL::r4_ei ( log ( x ) );

  return value;
}
//****************************************************************************80

float SFL::r4_lngam ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_LNGAM evaluates the log of the absolute value of gamma of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_LNGAM, the logarithm of the absolute value of
...
//
{
  ...
  if ( xmax == 0.0E+00 )
  {
    xmax = SFL::r4_mach ( 2 ) / log ( SFL::r4_mach ( 2 ) );
    dxrel = sqrt ( SFL::r4_mach ( 4 ) );
  }
  ...
  if ( y <= 10.0E+00 )
  {
    value = log ( SFL::r4_abs ( SFL::r4_gamma ( x ) ) );
    return value;
  }
  ...
  if ( 0.0E+00 < x )
  {
    value = sq2pil + ( x - 0.5E+00 ) * log ( x ) - x + SFL::r4_lgmc ( y );
    return value;
  }

  sinpiy = SFL::r4_abs ( sin ( pi * y ) );
  ...
  value = sqpi2l + ( x - 0.5E+00 ) * log ( y ) - x 
    - log ( sinpiy ) - SFL::r4_lgmc ( y );

  if ( SFL::r4_abs ( ( x - SFL::r4_aint ( x - 0.5E+00 ) ) * value / x ) < dxrel )
  {
    cerr << "\n";
    cerr << "SFL::r4_LNGAM - Warning!\n";
...
float SFL::r4_lnrel ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_LNREL evaluates log ( 1 + X ) for an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_LNREL, the value of log ( 1 + X ).
//
{
  ...
  if ( nlnrel == 0 )
  {
    nlnrel = SFL::r4_inits ( alnrcs, 23, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xmin = - 1.0E+00 + sqrt ( SFL::r4_mach ( 4 ) );
  }

  if ( x <= - 1.0E+00 )
  {
    cerr << "\n";
    cerr << "SFL::r4_LNREL - Fatal error!\n";
    cerr << "  X <= -1.\n";
    exit ( 1 );
  }
  else if ( x < xmin )
  {
    cerr << "\n";
    cerr << "SFL::r4_LNREL - Warning!\n";
    cerr << "  Result is less than half precision.\n";
    cerr << "  X is too close to - 1.\n";
  }

  if ( SFL::r4_abs ( x ) <= 0.375E+00 )
  {
    value = x * ( 1.0E+00 - x * SFL::r4_csevl ( x / 0.375E+00, alnrcs, nlnrel ) );
  }
  else
  {
    value = log ( 1.0E+00 + x );
...
float SFL::r4_log ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_LOG evaluates the logarithm of an R4.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the evaluation point.
//
//    Output, float SFL::r4_LOG, the logarithm of X.
//
{
  ...
  if ( nterms == 0 )
  {
    nterms = SFL::r4_inits ( alncs, 6, 28.9E+00 * SFL::r4_mach ( 3 ) );
  }

  if ( x <= 0.0E+00 )
  {
    cerr << "\n";
    cerr << "SFL::r4_LOG - Fatal error!\n";
    cerr << "  X <= 0.0\n";
    exit ( 1 );
  }
  ...
  value = 0.625E+00 * xn + ( aln2 * xn + alncen[ntrval-1] 
    + 2.0E+00 * t 
    + t * t2 * SFL::r4_csevl ( 578.0E+00 * t2 - 1.0, alncs, nterms ) );

  return value;
}
//****************************************************************************80

float SFL::r4_log10 ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_LOG10 evaluates the logarithm, base 10, of an R4.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the evaluation point.
//
//    Output, float SFL::r4_LOG10, the logarithm, base 10, of X.
//
{
...
float SFL::r4_mach ( int i )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_MACH returns single precision real machine constants.
//
//  Discussion:
//
...
//    SFL::r4_MACH will return the following values:
//
//      SFL::r4_MACH(1) = B^(EMIN-1), the smallest positive magnitude.
//      SFL::r4_MACH(2) = B^EMAX*(1-B^(-T)), the largest magnitude.
//      SFL::r4_MACH(3) = B^(-T), the smallest relative spacing.
//      SFL::r4_MACH(4) = B^(1-T), the largest relative spacing.
//      SFL::r4_MACH(5) = log10(B)
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    1 <= I <= 5.
//
//    Output, float SFL::r4_MACH, the value of the chosen parameter.
//
{
  float value;
...
void SFL::r4_machar ( long int *ibeta, long int *it, long int *irnd, long int *ngrd,
...
//  Purpose:
//
//    SFL::r4_MACHAR computes machine constants for R4 arithmetic.
//
//  Discussion:
//
...
//
  for ( ; ; )
  {
    ...
    if ( ( tmp1 == zero ) || ( SFL::r4_abs ( y ) >= ( *xmin ) ) )
    {
      break;
    }
...
float SFL::r4_max ( float x, float y )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_MAX returns the maximum of two R4's.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, float SFL::r4_MAX, the maximum of X and Y.
//
{
  float value;
...
float SFL::r4_min ( float x, float y )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_MIN returns the minimum of two R4's..
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license.
...
float SFL::r4_mod ( float x, float y )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_MOD returns the remainder of R4 division.
//
//  Discussion:
//
//    If
...
//  Example:
//
//        X         Y     SFL::r4_MOD   SFL::r4_MOD  Factorization
//
//      107        50       7     107 =  2 *  50 + 7
//      107       -50       7     107 = -2 * -50 + 7
//     -107        50      -7    -107 = -2 *  50 - 7
...
//
//    Output, float SFL::r4_MOD, the remainder when X is divided by Y.
//
{
  float value;
  ...
  {
    cerr << "\n";
    cerr << "SFL::r4_MOD - Fatal error!\n";
    cerr << "  SFL::r4_MOD ( X, Y ) called with Y = " << y << "\n";
    exit ( 1 );
  }
...
float SFL::r4_mop ( int i )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_MOP returns the I-th power of -1 as an R4 value.
//
//  Discussion:
//
...
//
//    Output, float SFL::r4_MOP, the I-th power of -1.
//
{
  float value;
...
float SFL::r4_pak ( float y, int n )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_PAK packs a base 2 exponent into an R4.
//
//  Discussion:
//
//    This routine is almost the inverse of SFL::r4_UPAK.  It is not exactly 
//    the inverse, because abs(x) need not be between 0.5 and 1.0.  
//    If both SFL::r4_PAK and 2.0^n were known to be in range, we could compute
//    SFL::r4_PAK = x * 2.0^n .
//
//  Licensing:
//
...
//
//    Output, float SFL::r4_PAK, the packed value.
//
{
  ...
  if ( nmin == 0 )
  {
    ...
    if ( SFL::i4_mach ( 10 ) != 2 )
    {
      aln2b = SFL::r4_mach ( 5 ) * aln210;
    }
    nmin = aln2b * ( float ) ( SFL::i4_mach ( 12 ) );
    nmax = aln2b * ( float ) ( SFL::i4_mach ( 13 ) );
  }

  SFL::r4_upak ( y, value, ny );
...
float SFL::r4_poch ( float a, float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_POCH evaluates Pochhammer's function of R4 arguments.
//
//  Discussion:
//
...
//
//    Output, float SFL::r4_POCH, the Pochhammer function of A and X.
//
{
  float absa;
  float absax;
  ...
  if ( ax <= 0.0E+00 && SFL::r4_aint ( ax ) == ax )
  {
    if ( 0.0E+00 < a || SFL::r4_aint ( a ) != a )
    {
      cerr << "\n";
      cerr << "SFL::r4_POCH - Fatal error!\n";
...
//
    ...
    else if ( - 20.0E+00 <= SFL::r4_min ( a + x, a ) )
    {
      n = ( int ) ( x );
      value = SFL::r4_mop ( n ) * SFL::r4_fac ( - ( int ) ( a ) ) 
        / SFL::r4_fac ( - ( int ) ( a ) - n );
    }
    else
    {
      n = ( int ) ( x );
      value = SFL::r4_mop ( n ) * exp ( ( a - 0.5E+00 ) 
        * SFL::r4_lnrel ( x / ( a - 1.0E+00 ) ) 
        + x * log ( - a + 1.0E+00 - x ) - x 
        + SFL::r4_lgmc ( - a + 1.0E+00 ) 
        - SFL::r4_lgmc ( - a - x + 1.0E+00 ) );
    }
...
//
  ...
  absax = SFL::r4_abs ( a + x );
  absa = SFL::r4_abs ( a );

  if ( SFL::r4_max ( absax, absa ) <= 20.0E+00 )
  {
    value = SFL::r4_gamma ( a + x ) * SFL::r4_gamr ( a );
    return value;
  }

  if ( 0.5E+00 * absa < SFL::r4_abs ( x ) )
  {
    SFL::r4_lgams ( a + x, alngax, sgngax );
    SFL::r4_lgams ( a, alnga, sgnga );
...
//
  ...
  value = exp ( ( b - 0.5E+00 ) * SFL::r4_lnrel ( x / b ) 
    + x * log ( b + x ) - x + SFL::r4_lgmc ( b + x ) - SFL::r4_lgmc ( b ) );
  ...
  errpch = SFL::r4_abs ( x ) * ( 1.0E+00 + log ( b ) );
  den = cospix + cospia * sinpix / sinpia;
  err = ( SFL::r4_abs ( x ) * ( SFL::r4_abs ( sinpix ) 
    + SFL::r4_abs ( cospia * cospix / sinpia ) )
    + SFL::r4_abs ( a * sinpix ) / ( sinpia * sinpia ) ) * pi;
  err = errpch + err / SFL::r4_abs ( den );
...
float SFL::r4_poch1 ( float a, float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_POCH1 evaluates a quantity related to Pochhammer's symbol.
//
//  Discussion:
//
...
//    Input, float X, the evaluation point.
//
//    Output, float SFL::r4_POCH1, the value of the function.
//
{
  float absa;
  float absx;
  ...
  if ( sqtbig == 0.0E+00 )
  {
    sqtbig = 1.0E+00 / sqrt ( 24.0E+00 * SFL::r4_mach ( 1 ) );
    alneps = log ( SFL::r4_mach ( 3 ) );
  }
  ...
  if ( 0.1E+00 * absa < absx || 0.1E+00 < absx * log ( SFL::r4_max ( absa, 2.0E+00 ) ) )
  {
    value = SFL::r4_poch ( a, x );
  ...
  value = SFL::r4_exprel ( q ) * ( alnvar + q * poly1 ) + poly1;
//
...
//
  ...
  trig = sinpxx * SFL::r4_cot ( pi * b ) - 2.0E+00 * sinpx2 * ( sinpx2 / x );
...
float SFL::r4_power ( float a, float b )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_POWER evaluates A^B.
//
//  Discussion:
//
...
//
//    Output, float SFL::r4_POWER, the value of A^B.
//
{
  float value;
...
float SFL::r4_psi ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_PSI evaluates the psi function of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_PSI, the psi function of X.
//
 {
  ...
  if ( ntpsi == 0 )
  {
    ntpsi = SFL::r4_inits ( psics, 23, 0.1E+00 * SFL::r4_mach ( 3 ) );
    ntapsi = SFL::r4_inits ( apsics, 16, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xbig = 1.0E+00 / sqrt ( SFL::r4_mach ( 3 ) );
    dxrel = sqrt ( SFL::r4_mach ( 4 ) );
  }
  ...
  if ( y < 2.0E+00 )
  {
    ...
    y = x - ( float ) ( n );
    n = n - 1;
    value = SFL::r4_csevl ( 2.0E+00 * y - 1.0E+00, psics, ntpsi );
    ...
    if ( x < - 0.5E+00 &&
      SFL::r4_abs ( ( x - SFL::r4_aint ( x - 0.5E+00 ) ) / x ) < dxrel )
    {
      cerr << "\n";
      cerr << "SFL::r4_PSI - Warning!\n";
  ...
  else
  {
    if ( y < xbig )
    {
      aux = SFL::r4_csevl ( 8.0E+00 / y / y - 1.0E+00, apsics, ntapsi );
    }
    else
    {
      aux = 0.0E+00;
    ...
    if ( x < 0.0E+00 )
    {
      value = log ( SFL::r4_abs ( x ) ) - 0.5E+00 / x + aux 
        - pi * SFL::r4_cot ( pi * x );
    }
    else if ( 0.0E+00 < x )
    {
...
float SFL::r4_rand ( float r )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_RAND is a portable pseudorandom number generator.
//
//  Discussion:
//
...
//    SFL::r4_RAND provided the arithmetic is done exactly.
//
//    Output, real SFL::r4_RAND, a pseudo-random number between 0.0 and 1.0.
//
{
  static int ia0 = 1029;
  static int ia1 = 1536;
  ...
  if ( 0.0 < r )
  {
    ix1 = ( int ) ( SFL::r4_mod ( r, 1.0E+00 ) * 4194304.0 + 0.5E+00 );
    ix0 = ( ix1 % 2048 );
    ix1 = ( ix1 - ix0 ) / 2048;
  }
...
float SFL::r4_randgs ( float xmean, float sd )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_RANDGS generates a normally distributed random number.
//
//  Discussion:
//
...
//
//    Output, float SFL::r4_RANDGS, a normally distributed random number.
//
{
  int i;
  float value;
  ...
  for ( i = 1; i <= 12; i++ )
  {
    value = value + SFL::r4_rand ( 0.0E+00 );
  }
...
float SFL::r4_random ( float t[], int n )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_RANDOM is a portable pseudorandom number generator.
//
//  Discussion:
//
...
//
//    Output, SFL::r4_RANDOM, a random number between 0.0 and 1.0.
//
{
  float dummy;
  ...
  if ( n != nold )
  {
    ...
    if ( n < 0 )
    {
      dummy = SFL::r4_rand ( t[nold] );
    }
    else
    {
      for ( i = 1; i <= nold; i++ )
      {
        t[i-1] = SFL::r4_rand ( 0.0E+00 );
      }
      t[nold] = SFL::r4_rand ( 0.0E+00 );
    }
  }
  ...
  t[nold] = t[j-1];
  value = t[j-1];
  t[j-1] = SFL::r4_rand ( 0.0E+00 );

  return value;
}
//****************************************************************************80

float SFL::r4_ranf ( float sw )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_RANF is a driver for SFL::r4_RANDOM.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    random number.
//
//    Output, float SFL::r4_RANF, the random value.
//
{
  int i;
...
//  Purpose:
//
//    SFL::r4_REN is a simple random number generator.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//  Parameters:
//
//    Output, float SFL::r4_REN, the random value.
//
{
...
float SFL::r4_shi ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_SHI evaluates the hyperbolic sine integral Shi of an R4 argument.
//
//  Discussion:
//
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_SHI, the hyperbolic sine integral Shi evaluated at X.
//
{
  float absx;
  static int nshi = 0;
  ...
  if ( nshi == 0 )
  {
    nshi = SFL::r4_inits ( shics, 7, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( SFL::r4_mach ( 3 ) );
  }
  ...
  {
    value = x * ( 1.0E+00 
      + SFL::r4_csevl ( 128.0E+00 * x * x / 9.0E+00 - 1.0E+00, 
      shics, nshi ) );
  }
  else
  {
...
float SFL::r4_si ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_SI evaluates the sine integral Si of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_SI, the sine integral Si evaluated at X.
//
{
  float absx;
  float cosx;
  ...
  if ( nsi == 0 )
  {
    nsi = SFL::r4_inits ( sics, 12, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( SFL::r4_mach ( 3 ) );
  }
  ...
  {
    value = x * ( 0.75E+00 
      + SFL::r4_csevl ( ( x * x - 8.0E+00 ) * 0.125E+00, sics, nsi ) );
  }
  else
  {
    SFL::r4_sifg ( absx, f, g );
    cosx = cos ( absx );
...
void SFL::r4_sifg ( float x, float &f, float &g )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_SIFG is a utility routine.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
{
  ...
  if ( nf1 == 0 )
  {
    tol = 0.1E+00 * SFL::r4_mach ( 3 );
    nf1 = SFL::r4_inits ( f1cs, 20, tol );
    nf2 = SFL::r4_inits ( f2cs, 29, tol );
    ng1 = SFL::r4_inits ( g1cs, 21, tol );
    ng2 = SFL::r4_inits ( g2cs, 34, tol );
    xbig = sqrt ( 1.0E+00 / SFL::r4_mach ( 3 ) );
    xmaxf = exp ( SFL::r4_min ( - log ( SFL::r4_mach ( 1 ) ), 
      log ( SFL::r4_mach ( 2 ) ) ) - 0.01E+00 );
    xmaxg = 1.0E+00 / sqrt ( SFL::r4_mach ( 1 ) );
    xbnd = sqrt ( 50.0E+00 );
  }
  ...
  if ( x <= xbnd )
  {
    f = ( 1.0E+00 + SFL::r4_csevl ( ( 1.0E+00 / x / x - 0.04125E+00 )
      / 0.02125E+00, f1cs, nf1 ) ) / x;
    g = ( 1.0E+00 + SFL::r4_csevl ( ( 1.0E+00 / x / x - 0.04125E+00 )
      / 0.02125E+00, g1cs, ng1 ) ) / x / x;
  }
  else if ( x <= xbig )
  {
    f = ( 1.0E+00 + SFL::r4_csevl ( 100.0E+00 / x / x - 1.0E+00, 
      f2cs, nf2 ) ) / x;
    g = ( 1.0E+00 + SFL::r4_csevl ( 100.0E+00 / x / x - 1.0E+00, 
      g2cs, ng2) ) / x / x;
  }
  else
  {
...
float SFL::r4_sign ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_SIGN returns the sign of an R4.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license.
...
//
//    Output, float SFL::r4_SIGN, the sign of X.
//
{
  float value;
...
float SFL::r4_sin ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_SIN evaluates the sine of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_SIN, the sine of X.
//
{
  float f;
  int n2;
...
//
  ...
  {
    ntsn = SFL::r4_inits ( sincs, 10, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xsml = sqrt ( 6.0E+00 * SFL::r4_mach ( 3 ) );
    xmax = 1.0E+00 / SFL::r4_mach ( 4 );
    xwarn = sqrt ( xmax );
  }
...
float SFL::r4_sin_deg ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_SIN_DEG evaluates the sine of an R4 argument in degrees.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument in degrees.
//
//    Output, float SFL::r4_SIN_DEG, the sine of X.
//
{
  int n;
  static float raddeg = 0.017453292519943296E+00;
...
float SFL::r4_sinh ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_SINH evaluates the hyperbolic sine of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_SINH, the hyperbolic sine of X.
//
{
  int nterms = 0;
  ...
  if ( nterms == 0 )
  {
    nterms = SFL::r4_inits ( sinhcs, 8, 0.1E+00 * SFL::r4_mach ( 3 ) );
    sqeps = sqrt ( 6.0E+00 * SFL::r4_mach ( 3 ) );
    ymax = 1.0E+00 / sqrt ( SFL::r4_mach ( 3 ) );
  }
  ...
  {
    value = x * ( 1.0E+00 
      + SFL::r4_csevl ( 2.0E+00 * x * x - 1.0E+00, sinhcs, nterms ) );
  }
  else
  {
    y = exp ( y );
...
float SFL::r4_spence ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_SPENCE evaluates a form of Spence's function for an R4 argument.
//
//  Discussion:
//
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_SPENCE, Spence's function evaluated at X.
//
{
  float aln;
  static int nspenc = 0;
  ...
  if ( nspenc == 0 )
  {
    nspenc = SFL::r4_inits ( spencs, 19, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xbig = 1.0E+00 / SFL::r4_mach ( 3 );
  }
  ...
  else if ( x <= - 1.0E+00 )
  {
    ...
    value = - pi26 - 0.5E+00 * aln * ( 2.0E+00 
      * log ( - x ) - aln ) + ( 1.0E+00 + SFL::r4_csevl (
  ...
  else if ( x <= 0.0E+00 )
  {
    value = - 0.5E+00 * log ( 1.0E+00 - x ) 
      * log ( 1.0E+00 - x ) - x * ( 1.0E+00 + SFL::r4_csevl ( 
      4.0E+00 * x / ( x - 1.0E+00 ) - 1.0E+00, spencs, nspenc ) ) 
      / ( x - 1.0E+00 );
  }
  else if ( x <= 0.5E+00 )
  {
    value = x * ( 1.0E+00 + SFL::r4_csevl ( 4.0E+00 * x - 1.0E+00, 
      spencs, nspenc ) );
  }
  else if ( x < 1.0E+00 )
  {
    value = pi26 - log ( x ) * log ( 1.0E+00 - x )
      - ( 1.0E+00 - x ) * ( 1.0E+00 + SFL::r4_csevl ( 4.0E+00 
  ...
  else if ( x <= 2.0E+00 )
  {
    value = pi26 - 0.5E+00 * log ( x ) 
      * log ( ( x - 1.0E+00 ) * ( x - 1.0E+00 ) / x )
      + ( x - 1.0E+00 ) * ( 1.0E+00 + SFL::r4_csevl ( 4.0E+00 
      * ( x - 1.0E+00 ) / x - 1.0E+00, spencs, nspenc ) ) / x;
  }
  else if ( x < xbig )
  {
    value = 2.0E+00 * pi26 - 0.5E+00 * log ( x ) * log ( x )
      - ( 1.0E+00 + SFL::r4_csevl ( 4.0E+00 / x - 1.0E+00, spencs, 
      nspenc ) ) / x;
  }
  else
  {
...
float SFL::r4_sqrt ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_SQRT computes the square root of an R4.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the number whose square root is desired.
//
//    Output, float SFL::r4_SQRT, the square root of X.
//
{
  int irem;
  int iter;
  ...
  if ( niter == 0 )
  {
    niter = 1.443E+00 * SFL::r4_log ( - 0.104E+00 
      * SFL::r4_log ( 0.1E+00 * SFL::r4_mach ( 3 ) ) ) + 1.0E+00;
  }

  if ( x < 0.0E+00 )
  {
    cerr << "\n";
    cerr << "SFL::r4_SQRT - Fatal error!\n";
    cerr << "  X < 0.0\n";
    exit ( 1 );
  }
  else if ( x == 0.0E+00 )
  ...
  else
  {
    ...
    value = SFL::r4_pak ( sqrt2[irem-1] * value, ixpnt );
  }
  return value;
}
//****************************************************************************80

float SFL::r4_tan ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_TAN evaluates the tangent of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_TAN, the tangent of X.
//
{
  float ainty;
  float ainty2;
  ...
  if ( nterms == 0 )
  {
    nterms = SFL::r4_inits ( tancs, 11, 0.1E+00 * SFL::r4_mach ( 3 ) );
    xmax = 1.0E+00 / SFL::r4_mach ( 4 );
    xsml = sqrt ( 3.0E+00 * SFL::r4_mach ( 3 ) );
    sqeps = sqrt ( SFL::r4_mach ( 4 ) );
  }
...
//
  ...
  if ( 1.0E+00 - y < SFL::r4_abs ( x ) * sqeps )
  {
    cerr << "\n";
    cerr << "SFL::r4_TAN - Warning!\n";
  ...
  if ( y <= 0.25E+00 )
  {
    ...
    {
      value = y * ( 1.5E+00 
        + SFL::r4_csevl ( 32.0E+00 * y * y - 1.0E+00, tancs, nterms ) );
    }
  }
  else if ( y <= 0.5E+00 )
  {
    value = 0.5E+00 * y * ( 1.5E+00 
      + SFL::r4_csevl ( 8.0E+00 * y * y - 1.0E+00, tancs, nterms ) );
  ...
  else
  {
    value = 0.25E+00 * y * ( 1.5E+00 
      + SFL::r4_csevl ( 2.0E+00 * y * y - 1.0E+00, tancs, nterms ) );
    value = 2.0E+00 * value / ( 1.0E+00 - value * value );
    value = 2.0E+00 * value / ( 1.0E+00 - value * value );
  }
...
float SFL::r4_tanh ( float x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_TANH evaluates the hyperbolic tangent of an R4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, float X, the argument.
//
//    Output, float SFL::r4_TANH, the hyperbolic tangent of X.
//
{
  static int nterms = 0;
  static float sqeps = 0.0;
  ...
  if ( nterms == 0 )
  {
    nterms = SFL::r4_inits ( tanhcs, 17, 0.1E+00 * SFL::r4_mach ( 3 ) );
    sqeps = sqrt ( 3.0E+00 * SFL::r4_mach ( 3 ) );
    xmax = - 0.5E+00 * log ( SFL::r4_mach ( 3 ) );
  }
  ...
  {
    value = x * ( 1.0E+00 
      + SFL::r4_csevl ( 2.0E+00 * x * x - 1.0E+00, tanhcs, nterms ) );
  }
  else if ( y <= xmax )
  {
    y = exp ( y );
...
void SFL::r4_upak ( float x, float &y, int &n )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r4_UPAK unpacks an R4 into a mantissa and exponent.
//
//  Discussion:
//
...
double SFL::r8_ai ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_AI evaluates the Airy function Ai of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, double SFL::r8_AI, the Airy function Ai of X.
//
{
  ...
  if ( naif == 0 )
  {
    naif = SFL::r8_inits ( aifcs, 13, 0.1 * SFL::r8_mach ( 3 ) );
    naig = SFL::r8_inits ( aigcs, 13, 0.1 * SFL::r8_mach ( 3 ) );
    x3sml = pow ( SFL::r8_mach ( 3 ), 0.3334 );
    xmax = pow ( - 1.5 * log ( SFL::r8_mach ( 1 ) ), 0.6667 );
    xmax = xmax - xmax * log ( xmax ) / 
  ...
  if ( x < - 1.0 )
  {
    SFL::r8_aimp ( x, xm, theta );
    value = xm * cos ( theta );
  }
  else if ( abs ( x ) <= x3sml )
  {
    z = 0.0;
    value = 0.375 + ( SFL::r8_csevl ( z, aifcs, naif ) 
      - x * ( 0.25 + SFL::r8_csevl ( z, aigcs, naig ) ) );
  }
  else if ( x <= 1.0 )
  {
    z = x * x * x;
  ...
  {
    value = SFL::r8_aie ( x ) * exp ( - 2.0 * x * sqrt ( x ) / 3.0 );
  }
  else
  {
    value = 0.0;
...
double SFL::r8_aid ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_AID evaluates the derivative of the Airy function Ai of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_AID, the derivative of the Airy function 
//    Ai of X.
//
{
  static double aifcs[13] = {
  ...
  if ( naif == 0 )
  {
    eta = 0.1 * SFL::r8_mach ( 3 );
    naif = SFL::r8_inits ( aifcs, 13, eta );
    naig = SFL::r8_inits ( aigcs, 13, eta );
    x3sml = pow ( SFL::r8_mach ( 3 ), 0.3334 );
    x2sml = sqrt ( SFL::r8_mach ( 3 ) );
  }

  if ( x < - 1.0 )
  {
    SFL::r8_admp ( x, xn, phi );
    value = xn * cos ( phi );
  }
  else if ( abs ( x ) <= x2sml )
  {
    x2 = 0.0;
    x3 = 0.0;
    value = ( x2 * ( 0.125 + SFL::r8_csevl ( x3, aifcs, naif ) ) 
      - SFL::r8_csevl ( x3, aigcs, naig ) ) - 0.25;
  }
  else if ( abs ( x ) <= x3sml )
  {
    x2 = x * x;
  ...
  {
    value = SFL::r8_aide ( x ) * exp ( - 2.0 * x * sqrt ( x ) / 3.0 );
  }
  return value;
}
//****************************************************************************80

double SFL::r8_aide ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_AIDE: exponentially scaled derivative, Airy function Ai of an R8 argument.
//
//  Discussion:
//
//    if X <= 0,
//      SFL::r8_AIDE ( X ) = SFL::r8_AID ( X )
//    else
//      SFL::r8_AIDE ( X ) = SFL::r8_AID ( X ) * exp ( 2/3 * X^(3/2) )
//
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_AIDE, the exponentially scaled derivative of 
//    the Airy function Ai of X.
//
{
  double aifcs[13] = {
  ...
  if ( naif == 0 )
  {
    ...
    naif = SFL::r8_inits ( aifcs, 13, eta );
    naig = SFL::r8_inits ( aigcs, 13, eta );
    naip1 = SFL::r8_inits ( aip1cs, 57, eta );
    naip2 = SFL::r8_inits ( aip2cs, 37, eta );
    x2sml = sqrt ( eta );
    x3sml = pow ( eta, 0.3333 );
    x32sml = 1.3104 * x3sml * x3sml;
    xbig = pow ( SFL::r8_mach ( 2 ), 0.6666 );
  }
  ...
  else if ( x <= 4.0 )
  {
    ...
    value = ( - 0.28125 - SFL::r8_csevl ( z, aip1cs, naip1 ) ) * sqrt ( sqrtx );
  }
  else if ( x < xbig )
  {
    sqrtx = sqrt ( x );
    z = 16.0  / ( x * sqrtx ) - 1.0;
    value = ( - 0.28125 - SFL::r8_csevl ( z, aip2cs, naip2 ) ) * sqrt ( sqrtx );
  }
  else
  {
    sqrtx = sqrt ( x );
...
double SFL::r8_aie ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_AIE evaluates the exponentially scaled Airy function Ai of an R8 argument.
//
//  Discussion:
//
//    if X <= 0,
//      SFL::r8_AIE ( X ) = SFL::r8_AI ( X )
//    else
//      SFL::r8_AIE ( X ) = SFL::r8_AI ( X ) * exp ( 2/3 * X^(3/2) )
//
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_AIE, the exponentially scaled Airy function Ai of X.
//
{
  static double aifcs[13] = {
    -0.37971358496669997496197089469414E-01,
  ...
  {
    z = 0.0;
    value = 0.3750 + ( SFL::r8_csevl ( z, aifcs, naif ) 
      - x * ( 0.25 + SFL::r8_csevl ( z, aigcs, naig ) ) );
  }
  else if ( abs ( x ) <= x3sml )
  {
...
void SFL::r8_aimp ( double x, double &ampl, double &theta )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_AIMP evaluates the modulus and phase of the Airy function.
//
//  Description:
//
//    This function must only be called when X <= -1.0.
...
//
{
  ...
  {
    eta = 0.1 * SFL::r8_mach ( 3 );
    nam20 = SFL::r8_inits ( am20cs, 57, eta );
    nath0 = SFL::r8_inits ( ath0cs, 53, eta );
    nam21 = SFL::r8_inits ( am21cs, 60, eta );
    nath1 = SFL::r8_inits ( ath1cs, 58, eta );
    nam22 = SFL::r8_inits ( am22cs, 74, eta );
    nath2 = SFL::r8_inits ( ath2cs, 72, eta );
    xsml = - pow ( 128.0 / SFL::r8_mach ( 3 ), 0.3333 );
  }

  if ( x <= xsml )
  {
    z = 1.0;
    ampl = 0.3125 + SFL::r8_csevl ( z, am20cs, nam20 );
    theta = - 0.625 + SFL::r8_csevl ( z, ath0cs, nath0 );
  }
  else if ( x < - 4.0 )
  {
    z = 128.0 / x / x / x + 1.0;
  ...
  else if ( x < - 2.0 )
  {
    ...
    ampl = 0.3125 + SFL::r8_csevl ( z, am21cs, nam21 );
    theta = - 0.625 + SFL::r8_csevl ( z, ath1cs, nath1 );
  }
  else if ( x <= - 1.0 )
  {
    z = ( 16.0 / x / x / x + 9.0 ) / 7.0;
    ampl = 0.3125 + SFL::r8_csevl ( z, am22cs, nam22 );
    theta = - 0.625 + SFL::r8_csevl ( z, ath2cs, nath2 );
  }
  else
  {
    cerr << "\n";
    cerr << "SFL::r8_AIMP - Fatal error!\n";
    cerr << "  -1.0 < X.\n";
    exit ( 1 );
  }
...
double SFL::r8_aint ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_AINT truncates an R8 argument to an integer.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_AINT, the truncated version of X.
//
{
  double value;
...
void SFL::r8_b0mp ( double x, double &ampl, double &theta )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_B0MP evaluates the modulus and phase for the Bessel J0 and Y0 functions.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
{
  ...
  {
    eta = 0.1 * SFL::r8_mach ( 3 );
    nbm0 = SFL::r8_inits ( bm0cs, 37, eta );
    nbt02 = SFL::r8_inits ( bt02cs, 39, eta );
    nbm02 = SFL::r8_inits ( bm02cs, 40, eta );
    nbth0 = SFL::r8_inits ( bth0cs, 44, eta );
    xmax = 1.0 / SFL::r8_mach ( 4 );
  }

  if ( x < 4.0 )
  {
    cerr << "\n";
    cerr << "SFL::r8_B0MP - Fatal error!\n";
    cerr << "  X < 4.\n";
    exit ( 1 );
  }
  else if ( x <= 8.0 )
  {
    z = ( 128.0 / x / x - 5.0 ) / 3.0;
    ampl = ( 0.75 + SFL::r8_csevl ( z, bm0cs, nbm0 ) ) / sqrt ( x );
    theta = x - pi4 + SFL::r8_csevl ( z, bt02cs, nbt02 ) / x;
  }
  else
  {
    z = 128.0 / x / x - 1.0;
    ampl = ( 0.75 + SFL::r8_csevl ( z, bm02cs, nbm02) ) / sqrt ( x );
    theta = x - pi4 + SFL::r8_csevl ( z, bth0cs, nbth0 ) / x;
  }
  return;
}
//****************************************************************************80

void SFL::r8_b1mp ( double x, double &ampl, double &theta )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_B1MP evaluates the modulus and phase for the Bessel J1 and Y1 functions.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
{
  ...
  {
    eta = 0.1 * SFL::r8_mach ( 3 );
    nbm1 = SFL::r8_inits ( bm1cs, 37, eta );
    nbt12 = SFL::r8_inits ( bt12cs, 39, eta );
    nbm12 = SFL::r8_inits ( bm12cs, 40, eta );
    nbth1 = SFL::r8_inits ( bth1cs, 44, eta );
    xmax = 1.0 / SFL::r8_mach ( 4 );
  }
  ...
  {
    z = ( 128.0 / x / x - 5.0 ) / 3.0;
    ampl = ( 0.75 + SFL::r8_csevl ( z, bm1cs, nbm1 ) ) / sqrt ( x );
    theta = x - 3.0 * pi4 + SFL::r8_csevl ( z, bt12cs, nbt12 ) / x;
  }
  else
  {
    z = 128.0 / x / x - 1.0;
    ampl = ( 0.75 + SFL::r8_csevl ( z, bm12cs, nbm12 ) ) / sqrt ( x );
    theta = x - 3.0 * pi4 + SFL::r8_csevl ( z, bth1cs, nbth1 ) / x;
  }
  return;
}
//****************************************************************************80

double SFL::i0 (const double x )

{
  ...
  if ( nti0 == 0 )
  {
    nti0 = SFL::r8_inits ( bi0cs, 18, 0.1 * SFL::r8_mach ( 3 ) );
    xsml = sqrt ( 8.0 * SFL::r8_mach ( 3 ) );
    xmax = log ( SFL::r8_mach ( 2 ) );
  }
  ...
  {
    value = 2.75 + SFL::r8_csevl ( y * y / 4.5 - 1.0, bi0cs, nti0 );
  }
  else if ( y <= xmax )
  {
    value = exp ( y ) * SFL::i0e( x );
  }
  else
  {
    cerr << "\n";
...
double SFL::i0e (const double x )
{
  ...
  {
    eta = 0.1 * SFL::r8_mach ( 3 );
    nti0 = SFL::r8_inits ( bi0cs, 18, eta );
    ntai0 = SFL::r8_inits ( ai0cs, 46, eta );
    ntai02 = SFL::r8_inits ( ai02cs, 69, eta );
    xsml = sqrt ( 8.0 * SFL::r8_mach ( 3 ) );
  }
  ...
  {
    value = exp ( - y ) * ( 2.75 
      + SFL::r8_csevl ( y * y / 4.5 - 1.0, bi0cs, nti0 ) );
  }
  else if ( y <= 8.0 )
  {
    value = ( 0.375
      + SFL::r8_csevl ( ( 48.0 / y - 11.0 ) / 5.0, ai0cs, ntai0 ) ) / sqrt ( y );
  }
  else
  {
    value = ( 0.375 
      + SFL::r8_csevl ( 16.0 / y - 1.0, ai02cs, ntai02 ) )  / sqrt ( y );
  }
  return value;
}
...

{
  ...
  if ( nti1 == 0 )
  {
    nti1 = SFL::r8_inits ( bi1cs, 17, 0.1 * SFL::r8_mach ( 3 ) );
    xmin = 2.0 * SFL::r8_mach ( 1 );
    xsml = sqrt ( 8.0 * SFL::r8_mach ( 3 ) );
    xmax = log ( SFL::r8_mach ( 2 ) );
  }
  ...
  {
    value = x * ( 0.875 + SFL::r8_csevl ( y * y / 4.5 - 1.0, bi1cs, nti1 ) );
  }
  else if ( y <= xmax )
  {
    value = exp ( y ) * SFL::i1e ( x );
  }
  else
  {
    cerr << "\n";
    cerr << "SFL::i1 - Fatal error!\n";
    cerr << "  Result overflows.\n";
    exit ( 1 );
  }
  return value;
...
double SFL::i1e (const double x )
{
  ...
  {
    eta = 0.1 * SFL::r8_mach ( 3 );
    nti1 = SFL::r8_inits ( bi1cs, 17, eta );
    ntai1 = SFL::r8_inits ( ai1cs, 46, eta );
    ntai12 = SFL::r8_inits ( ai12cs, 69, eta );
    xmin = 2.0 * SFL::r8_mach ( 1 );
    xsml = sqrt ( 8.0 * SFL::r8_mach ( 3 ) );
  }
  ...
  {
    value = ( 0.375 + SFL::r8_csevl ( ( 48.0 / y - 11.0) / 5.0, 
      ai1cs, ntai1 ) ) / sqrt ( y );
    if ( x < 0.0 )
    {
      value = - value;
  ...
  {
    value = ( 0.375 + SFL::r8_csevl ( 16.0 / y - 1.0, ai12cs, ntai12 ) ) / sqrt ( y );
    if ( x < 0.0 )
    {
      value = - value;
    }
...

{
  ...
  if ( ntj0 == 0 )
  {
    ntj0 = SFL::r8_inits ( bj0cs, 19, 0.1 * SFL::r8_mach ( 3 ) );
    xsml = sqrt ( 4.0 * SFL::r8_mach ( 3 ) );
  }
  ...
  {
    value = SFL::r8_csevl ( 0.125 * y * y - 1.0, bj0cs, ntj0 );
  }
  else
  {
    SFL::r8_b0mp ( y, ampl, theta );
    value = ampl * cos ( theta );
  }
  return value;
}

double SFL::j1 (const double x )

{
  ...
  if ( ntj1 == 0 )
  {
    ntj1 = SFL::r8_inits ( bj1cs, 19, 0.1 * SFL::r8_mach ( 3 ) );
    xsml = sqrt ( 4.0 * SFL::r8_mach ( 3 ) );
    xmin = 2.0 * SFL::r8_mach ( 1 );
  }
  ...
  {
    value = x * ( 0.25 + SFL::r8_csevl ( 0.125 * y * y - 1.0, bj1cs, ntj1 ) );
  }
  else
  {
    SFL::r8_b1mp ( y, ampl, theta );
    if ( x < 0.0 )
    {
      value = - ampl * cos ( theta );
    }
...
double SFL::k0 (const double x )
{
  ...
  if ( ntk0 == 0 )
  {
    ntk0 = SFL::r8_inits (bk0cs, 16, 0.1 * SFL::r8_mach ( 3 ) );
    xsml = sqrt ( 4.0 * SFL::r8_mach ( 3 ) );
    xmax = - log ( SFL::r8_mach ( 1 ) );
  ...
  {
    y = 0.0;
    value = - log ( 0.5 * x ) * i0 ( x )  
      - 0.25 + SFL::r8_csevl ( 0.5 * y - 1.0, bk0cs, ntk0 );
  }
  else if ( x <= 2.0 )
  {
    y = x * x;
...

{
  ...
  {
    eta = 0.1 * SFL::r8_mach ( 3 );
    ntk0 = SFL::r8_inits ( bk0cs, 16, eta );
    ntak0 = SFL::r8_inits ( ak0cs, 38, eta );
    ntak02 = SFL::r8_inits ( ak02cs, 33, eta );
    xsml = sqrt ( 4.0 * SFL::r8_mach ( 3 ) );
  }
  ...
  {
    y = 0.0;
    value = exp ( x ) * ( - log ( 0.5 * x ) * i0 ( x ) - 0.25 
      + SFL::r8_csevl ( 0.5 * y - 1.0, bk0cs, ntk0 ) );
  }
  else if ( x <= 2.0 )
  {
    y = x * x;
  ...
  {
    value = ( 1.25 + SFL::r8_csevl ( ( 16.0 / x - 5.0 ) / 3.0, ak0cs, 
      ntak0 ) ) / sqrt ( x );
  }
  else
  {
    value = ( 1.25 + SFL::r8_csevl ( 16.0 / x - 1.0, ak02cs, ntak02 ) ) / sqrt ( x );
  } 
  return value;
}

double SFL::k1 (const double x )
{
  ...
  if ( ntk1 == 0 )
  {
    ntk1 = SFL::r8_inits ( bk1cs, 16, 0.1 * SFL::r8_mach ( 3 ) );
    xmin = exp ( SFL::r8_max ( log ( SFL::r8_mach ( 1 ) ), 
      - log ( SFL::r8_mach ( 2 ) ) ) + 0.01 );
    xsml = sqrt ( 4.0 * SFL::r8_mach ( 3 ) );
    xmax = - log ( SFL::r8_mach ( 1 ) );
  ...
  {
    y = 0.0;
    value = log ( 0.5 * x ) * SFL::i1 ( x ) + ( 0.75 
      + SFL::r8_csevl ( 0.5 * y - 1.0, bk1cs, ntk1 ) ) / x;
  }
  else if ( x <= 2.0 )
  {
    y = x * x;
...
double SFL::k1e (const double x )
{
  ...
  {
    eta = 0.1 * SFL::r8_mach ( 3 );
    ntk1 = SFL::r8_inits ( bk1cs, 16, eta );
    ntak1 = SFL::r8_inits ( ak1cs, 38, eta );
    ntak12 = SFL::r8_inits ( ak12cs, 33, eta );
    xmin = exp ( SFL::r8_max ( log ( SFL::r8_mach ( 1 ) ), 
      - log ( SFL::r8_mach ( 2 ) ) ) + 0.01 );
    xsml = sqrt ( 4.0 * SFL::r8_mach ( 3 ) );
  }
  ...
  {
    y = 0.0;
    value = exp ( x ) * ( log ( 0.5 * x ) * SFL::i1 ( x )
      + ( 0.75 + SFL::r8_csevl ( 0.5 * y - 1.0, bk1cs, ntk1 ) ) / x );
  }
  else if ( x <= 2.0 )
  {
    y = x * x;
  ...
  {
    value = ( 1.25 
      + SFL::r8_csevl ( ( 16.0 / x - 5.0 ) / 3.0, ak1cs, ntak1 ) ) / sqrt ( x );
  }
  else
  {
    value = ( 1.25 +
      SFL::r8_csevl ( 16.0 / x - 1.0, ak12cs, ntak12 ) ) / sqrt ( x );
  }
  return value;
}
//****************************************************************************80

void SFL::r8_beskes ( double xnu, double x, int nin, double bke[] )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_BESKES: a sequence of exponentially scaled K Bessel functions at X.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
{
  ...
  {
    cerr << "\n";
    cerr << "SFL::r8_BESKES - Fatal error!\n";
    cerr << "  |XNU| must be less than 1.\n";
    exit ( 1 );
  }
  ...
  SFL::r8_knus ( v, x, bke[0], bknu1, iswtch );
  ...
  if ( direct < 0.0 )
  {
    SFL::r8_knus ( abs ( xnu + vincr ), x, bke[1], bknu1, iswtch );
  }
...
void SFL::r8_besks ( double xnu, double x, int nin, double bk[] )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_BESKS evaluates a sequence of K Bessel functions at X.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
double SFL::y0 (const double x )
{
  ...
  if ( nty0 == 0 )
  {
    nty0 = SFL::r8_inits ( by0cs, 19, 0.1 * SFL::r8_mach ( 3 ) );
    xsml = sqrt ( 4.0 * SFL::r8_mach ( 3 ) );
  }
  ...
  {
    y = 0.0;
    value = twodpi * ( alnhaf + log ( x ) ) * SFL::j0 ( x ) 
      + 0.375 + SFL::r8_csevl ( 0.125 * y - 1.0, by0cs, nty0 );
  }
  else if ( x <= 4.0 )
  {
    y = x * x;
    value = twodpi * ( alnhaf + log ( x ) ) * SFL::j0 ( x ) + 0.375 
      + SFL::r8_csevl ( 0.125 * y - 1.0, by0cs, nty0 );
  }
  else
  {
    SFL::r8_b0mp ( x, ampl, theta );
    value = ampl * sin ( theta );
  }
  return value;
}
...
double SFL::y1 (const double x )
{
  ...
  if ( nty1 == 0 )
  {
    nty1 = SFL::r8_inits ( by1cs, 20, 0.1 * SFL::r8_mach ( 3 ) );
    xmin = 1.571 * exp ( SFL::r8_max ( log ( SFL::r8_mach ( 1 ) ), 
      - log ( SFL::r8_mach ( 2 ) ) ) + 0.01 );
    xsml = sqrt ( 4.0 * SFL::r8_mach ( 3 ) );
  }
  ...
  {
    y = 0.0;
    value = twodpi * log ( 0.5 * x ) * SFL::j1 ( x ) 
      + ( 0.5 + SFL::r8_csevl ( 0.125 * y - 1.0, by1cs, nty1 ) ) / x;
  }
  else if ( x <= 4.0 )
  {
    y = x * x;
...
double SFL::r8_beta ( double a, double b )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_BETA evaluates the beta function of R8 arguments.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, double SFL::r8_BETA, the beta function of A and B.
//
{
  ...
  if ( xmax == 0.0 )
  {
    SFL::r8_gaml ( xmin, xmax );
    alnsml = log ( SFL::r8_mach ( 1 ) );
  }

  if ( a <= 0.0 || b <= 0.0 )
  {
    cerr << "\n";
    cerr << "SFL::r8_BETA - Fatal error!\n";
    cerr << "  A and B must be greater than 0.\n";
    exit ( 1 );
  }
  ...
  if ( a + b < xmax )
  {
    value = SFL::r8_gamma ( a ) * SFL::r8_gamma ( b ) / SFL::r8_gamma ( a + b );
    return value;
  }

  value = SFL::r8_lbeta ( a, b );
...
double SFL::r8_betai ( double x, double pin, double qin )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_BETAI evaluates the incomplete beta ratio of R8 arguments.
//
//  Discussion:
//
//    The incomplete Beta function ratio is the probability that a
...
//    0.0 < QIN.
//
//    Output, double SFL::r8_BETAI, the incomplete beta function ratio.
//
{
  ...
  {
    value = 0.0;
    xb = p * log ( SFL::r8_max ( y, sml ) ) - log ( p ) - SFL::r8_lbeta ( p, q );
    if ( alnsml < xb && y != 0.0 )
    {
      value = exp ( xb );
    }
  ...
  xb = p * log ( y ) - SFL::r8_lbeta ( ps, p ) - log ( p );
  ...
  else
  {
    ...
    if ( ps != 1.0 )
    {
      n = ( int ) ( SFL::r8_max ( alneps / log ( y ), 4.0 ) );
      for ( i = 1; i <= n; i++ )
      {
        xi = ( double ) ( i );
  ...
  if ( 1.0 < q )
  {
    xb = p * log ( y ) + q * log ( 1.0 - y ) 
      - SFL::r8_lbeta ( p, q ) - log ( q );
    ib = ( int ) ( SFL::r8_max ( xb / alnsml, 0.0 ) );
...
double SFL::r8_bi ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_BI evaluates the Airy function Bi of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_BI, the Airy function Bi of X.
//
{
  ...
  {
    eta = 0.1 * SFL::r8_mach ( 3 );
    nbif = SFL::r8_inits ( bifcs, 13, eta );
    nbig = SFL::r8_inits ( bigcs, 13, eta );
    nbif2 = SFL::r8_inits ( bif2cs, 15, eta );
    nbig2 = SFL::r8_inits ( big2cs, 15, eta );
    x3sml = pow ( eta, 0.3333 );
    xmax = pow ( 1.5 * log ( SFL::r8_mach ( 2 ) ), 0.6666 );
  }
  ...
  {
    z = 0.0;
    value = 0.625 + SFL::r8_csevl ( z, bifcs, nbif ) 
      + x * ( 0.4375 + SFL::r8_csevl ( z, bigcs, nbig ) );
  }
  else if ( x <= 1.0 )
  {
    z = x * x * x;
  ...
  {
    z = ( 2.0 * x * x * x - 9.0 ) / 7.0;
    value = 1.125 + SFL::r8_csevl ( z, bif2cs, nbif2 ) 
      + x * ( 0.625 + SFL::r8_csevl ( z, big2cs, nbig2 ) );
  }
  else
  {
    value = SFL::r8_bie ( x ) * exp ( 2.0 * x * sqrt ( x ) / 3.0 );
  }
  return value;
}
//****************************************************************************80

double SFL::r8_bid ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_BID evaluates the derivative of the Airy function Bi of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_BID, the derivative of the Airy function Bi of X.
//
{
  static double bif2cs[15] = {
  ...
  if ( nbif == 0 )
  {
    ...
    nbig = SFL::r8_inits ( bigcs, 13, eta );
    nbif2 = SFL::r8_inits ( bif2cs, 15, eta );
    nbig2 = SFL::r8_inits ( big2cs, 16, eta );
    x2sml = sqrt ( eta );
    x3sml = pow ( eta, 0.3333 );
    xmax = pow ( 1.5 * log ( SFL::r8_mach ( 2 ) ), 0.6666 );
  }
  ...
  else if ( abs ( x ) <= x2sml )
  {
    x2 = 0.0;
    x3 = 0.0;
    value = x2 * ( SFL::r8_csevl ( x3, bifcs, nbif ) + 0.25 ) 
      + SFL::r8_csevl ( x3, bigcs, nbig ) + 0.5;
  }
  else if ( abs ( x ) <= x3sml )
  {
    x2 = x * x;
  ...
  {
    z = ( 2.0 * x * x * x - 9.0 ) / 7.0;
    value = x * x * ( SFL::r8_csevl ( z, bif2cs, nbif2 ) + 0.25 ) 
      + SFL::r8_csevl ( z, big2cs, nbig2 ) + 0.5;
  }
  else
  {
    value = SFL::r8_bide ( x ) * exp ( 2.0 * x * sqrt ( x ) / 3.0 );
  }
  return value;
}
//****************************************************************************80

double SFL::r8_bide ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_BIDE: exponentially scaled derivative, Airy function Bi of an R8 argument.
//
//  Discussion:
//
//    if X < 0,
//      SFL::r8_BIDE ( X ) = SFL::r8_BID ( X )
//    else
//      SFL::r8_BIDE ( X ) = SFL::r8_BID ( X ) * exp ( - 2/3 * X^(3/2) )
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_BIDE, the exponentially scaled derivative of 
//    the Airy function Bi of X.
//
{
  ...
  if ( nbif == 0 )
  {
    ...
    nbif2 = SFL::r8_inits ( bif2cs, 15, eta );
    nbig2 = SFL::r8_inits ( big2cs, 16, eta );
    nbip1 = SFL::r8_inits ( bip1cs, 47, eta );
    nbip2 = SFL::r8_inits ( bip2cs, 88, eta );
    x2sml = sqrt ( eta );
    x3sml = pow ( eta, 0.3333 );
    x32sml = 1.3104 * x3sml * x3sml;
    xbig = pow ( SFL::r8_mach ( 2 ), 0.6666 );
  ...
  else if ( abs ( x ) <= x2sml )
  {
    x2 = 0.0;
    x3 = 0.0;
    value = x2 * ( SFL::r8_csevl ( x3, bifcs, nbif ) 
      + 0.25 ) + SFL::r8_csevl ( x3, bigcs, nbig ) + 0.5;
    if ( x32sml < x )
    {
      value = value * exp ( - 2.0 * x * sqrt ( x ) / 3.0 );
    }
  ...
  else if ( x <= 2.0 )
  {
    ...
    value = exp ( - 2.0 * x * sqrt ( x ) / 3.0 ) 
      * ( x * x * ( 0.25 + SFL::r8_csevl ( z, bif2cs, nbif2 ) ) 
      + 0.5 + SFL::r8_csevl ( z, big2cs, nbig2 ) );
  }
  else if ( x <= 4.0 )
  {
    sqrtx = sqrt ( x );
    z = atr / x / sqrtx + btr;
    value = ( 0.625 + SFL::r8_csevl ( z, bip1cs, nbip1 ) ) * sqrt ( sqrtx );
  }
  else if ( x <= xbig )
  {
    sqrtx = sqrt ( x );
    z = 16.0 / x / sqrtx - 1.0;
    value = ( 0.625 + SFL::r8_csevl ( z, bip2cs, nbip2 ) ) * sqrt ( sqrtx );
  }
  else
  {
    sqrtx = sqrt ( x );
...
double SFL::r8_bie ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_BIE evaluates the exponentially scaled Airy function Bi of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_BIE, the exponentially scaled Airy function Bi of X.
//
{
  static double atr = 8.75069057084843450880771988210148;
  static double bif2cs[15] = {
  ...
  else if ( x <= 2.0 )
  {
    z = ( 2.0 * x * x * x - 9.0 ) / 7.0;
    value = exp ( - 2.0 * x * sqrt ( x ) / 3.0 ) 
      * ( 1.125 + SFL::r8_csevl ( z, bif2cs, nbif2 ) 
      + x * ( 0.625 + SFL::r8_csevl ( z, big2cs, nbig2 ) ) );
  }
  else if ( x <= 4.0 )
  {
...
double SFL::r8_binom ( int n, int m )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_BINOM evaluates the binomial coefficient using R8 arithmetic.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, int N, M, the arguments.
//
//    Output, double SFL::r8_BINOM, the binomial coefficient.
//
{
  ...
  if ( bilnmx == 0.0 )
  {
    bilnmx = log ( SFL::r8_mach ( 2 ) ) - 0.0001;
    fintmx = 0.9 / SFL::r8_mach ( 3 );
  }

  if ( n < 0 )
  {
    cerr << "\n";
    cerr << "SFL::r8_BINOM - Fatal error!\n";
    cerr << "  N < 0.\n";
    exit ( 1 );
  }
  ...
  if ( k <= 20 && 
    ( double ) ( k ) * log ( ( double ) ( SFL::i4_max ( n, 1 ) ) ) <= bilnmx )
  {
    value = 1.0;
    for ( i = 1; i <= k; i++ )
    {
  ...
  else
  {
    ...
    corr = SFL::r8_lgmc ( xn ) - SFL::r8_lgmc ( xk ) - SFL::r8_lgmc ( xnk );

    value = xk * log ( xnk / xk ) 
      - xn * SFL::r8_lnrel ( - ( xk - 1.0 ) / xn )
...
double SFL::r8_chi ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_CHI evaluates the hyperbolic cosine integral of an R8 argument.
//
//  Discussion:
//
//    The hyperbolic cosine integral is defined by
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_CHI, the hyperbolic cosine integral
//    evaluated at X.
//
{
  double value;

  value = 0.5 * ( SFL::r8_ei ( x ) - SFL::r8_e1 ( x ) );

  return value;
}
//****************************************************************************80

double SFL::r8_chu ( double a, double b, double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_CHU evaluates the confluent hypergeometric function of R8 arguments.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_CHU, the function value.
//
{
  double a0;
  double aintb;
  ...
  if ( x == 0.0 )
  {
    ...
    value = SFL::r8_gamma ( 1.0 - b ) / SFL::r8_gamma ( 1.0 + a - b );
    return value;
  }

  if ( SFL::r8_max ( abs ( a ), 1.0 ) 
    * SFL::r8_max ( abs ( 1.0 + a - b ), 1.0 ) < 0.99 * abs ( x ) )
  {
    value = pow ( x, - a ) * SFL::r8_chu_scaled ( a, b, x );
    return value;
  }
//
//  The ascending series will be used, because the descending rational
...
//
  else
  {
    ...
    if ( 0 <= m )
    {
      ...
      sum = SFL::r8_gamma ( b - 1.0 ) * SFL::r8_gamr ( a )
        * pow ( x, ( double ) ( 1 - n ) ) * xtoeps * sum;
    }
  }
//
...
//
  ...
  factor = SFL::r8_mop ( n ) * SFL::r8_gamr ( 1.0 + a - b ) * pow ( x, xi );
  ...
  pochai = SFL::r8_poch ( a, xi );
  gamri1 = SFL::r8_gamr ( xi + 1.0 );
  gamrni = SFL::r8_gamr ( aintb + xi );
  b0 = factor * SFL::r8_poch ( a, xi - beps ) 
    * gamrni * SFL::r8_gamr ( xi + 1.0 - beps );
//
...
//
  ...
  {
    pch1ai = SFL::r8_poch1 ( a + xi, -beps );
    pch1i = SFL::r8_poch1 ( xi + 1.0 - beps, beps );
    c0 = factor * pochai * gamrni * gamri1 * (
      - SFL::r8_poch1 ( b + xi,- beps ) + pch1ai 
      - pch1i + beps * pch1ai * pch1i );
//
//  xeps1 = (1.0 - x^(-beps))/beps = (x^(-beps) - 1.0)/(-beps)
//
    xeps1 = alnx* SFL::r8_exprel ( - beps * alnx );
...
//  formulation is stable.
//
  a0 = factor * pochai * SFL::r8_gamr ( b + xi ) * gamri1 / beps;
  b0 = xtoeps * b0 / beps;
...
double SFL::r8_chu_scaled ( double a, double b, double z )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_CHU_SCALED: scaled confluent hypergeometric function of R8 arguments.
//
//  Discussion:
//
//    Evaluate, for large z, z^a * u(a,b,z)  where U is the logarithmic
...
//
//    Output, double SFL::r8_CHU_SCALED, the function value.
//
{
  double aa[4];
  double ab;
  ...
  if ( eps == 0.0 )
  {
    eps = 4.0 * SFL::r8_mach ( 4 );
    sqeps = sqrt ( SFL::r8_mach ( 4 ) );
  }
  ...
  cerr << "\n";
  cerr << "SFL::r8_CHU_SCALED - Fatal error!\n";
  cerr << "  No convergence after 300 terms.\n";
  exit ( 1 );
}
//****************************************************************************80

double SFL::r8_ci ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_CI evaluates the cosine integral Ci of an R8 argument.
//
//  Discussion:
//
//    The cosine integral is defined by
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_CI, the cosine integral Ci evaluated at X.
//
{
  ...
  if ( nci == 0 )
  {
    nci = SFL::r8_inits ( cics, 19, 0.1 * SFL::r8_mach ( 3 ) );
    xsml = sqrt ( SFL::r8_mach ( 3 ) );
  }

  if ( x <= 0.0 )
  {
    cerr << "\n";
    cerr << "SFL::r8_CI - Fatal error!\n";
    cerr << "  X <= 0.0.\n";
    exit ( 1 );
  }
  else if ( x <= xsml )
  {
    y = - 1.0;
    value = log ( x ) - 0.5 + SFL::r8_csevl ( y, cics, nci );
  }
  else if ( x <= 4.0 )
  {
    y = ( x * x - 8.0 ) * 0.125;
...
double SFL::r8_cin ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_CIN evaluates the alternate cosine integral Cin of an R8 argument.
//
//  Discussion:
//
//    CIN(X) = gamma + log(X) 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_CIN, the cosine integral Cin evaluated at X.
//
{
  double absx;
  ...
  if ( ncin == 0 )
  {
    ncin = SFL::r8_inits ( cincs, 18, 0.1 * SFL::r8_mach ( 3 ) );
    xmin = sqrt ( SFL::r8_mach ( 1 ) );
  }
  ...
  {
    value = SFL::r8_csevl ( ( x * x - 8.0 ) * 0.125, cincs, ncin ) * x * x;
  }
  else
  {
    SFL::r8_sifg ( absx, f, g );
    sinx = sin ( absx );
    value = - f * sinx + g * cos ( absx ) + log ( absx ) + eul;
  }
  return value;
...
double SFL::r8_cinh ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_CINH: alternate hyperbolic cosine integral Cinh of an R8 argument.
//
//  Discussion:
//
//    Cinh ( x ) = Integral ( 0 <= t <= x ) ( cosh ( t ) - 1 ) dt / t
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_CINH, the hyperbolic cosine integral Cinh
//    evaluated at X.
//
{
  double absx;
  ...
  if ( ncinh == 0 )
  {
    ncinh = SFL::r8_inits ( cinhcs, 16, 0.1 * SFL::r8_mach ( 3 ) );
    xsml = sqrt ( SFL::r8_mach ( 3 ) );
    xmin = 2.0 * sqrt ( SFL::r8_mach ( 1 ) );
  }
  ...
  {
    y = - 1.0;
    value = x * x * ( 0.25 + SFL::r8_csevl ( y, cinhcs, ncinh ) );
  }
  else if ( x <= 3.0 )
  {
    y = x * x / 4.5 - 1.0;
  ...
  {
    value = SFL::r8_chi ( absx ) - eul - log ( absx );
  }
  return value;
}
//****************************************************************************80
...
//  Purpose:
//
//    cos_DEG evaluates the cosine of an R8 argument in degrees.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, double cos_DEG, the cosine of X.
//
{
  int n;
...
double SFL::r8_cot ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_COT evaluates the cotangent of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_COT, the cotangent of X.
//
{
  double ainty;
  double ainty2;
  ...
  if ( nterms == 0 )
  {
    nterms = SFL::r8_inits ( cotcs, 15, 0.1 * SFL::r8_mach ( 3 ) );
    xmax = 1.0 / SFL::r8_mach ( 4 );
    xsml = sqrt ( 3.0 * SFL::r8_mach ( 3 ) );
    xmin = exp ( SFL::r8_max ( log ( SFL::r8_mach ( 1 ) ), 
      - log ( SFL::r8_mach ( 2 ) ) )  + 0.01 );
    sqeps = sqrt ( SFL::r8_mach ( 4 ) );
  }
...
//
  ...
  yrem = y - ainty;
  prodbg = 0.625 * ainty;
  ainty = SFL::r8_aint ( prodbg );
  y = ( prodbg - ainty ) + 0.625 * yrem + y * pi2rec;
  ainty2 = SFL::r8_aint ( y );
  ainty = ainty + ainty2;
  y = y - ainty2;
  ...
  {
    value = ( 0.5 + SFL::r8_csevl ( 32.0 * y * y - 1.0, cotcs, nterms ) ) / y;
  }
  else if ( y <= 0.5 )
  {
    value = ( 0.5 + SFL::r8_csevl ( 8.0 * y * y - 1.0, 
      cotcs, nterms ) ) / ( 0.5 * y );
  ...
  {
    value = ( 0.5 + SFL::r8_csevl ( 2.0 * y * y - 1.0, 
      cotcs, nterms ) ) / ( 0.25 * y );
    value = ( value * value - 1.0 ) * 0.5 / value;
    value = ( value * value - 1.0 ) * 0.5 / value;
  }
...
//****************************************************************************80
 ...
 double SFL::r8_csevl ( double x, double a[], int n )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_CSEVL evaluates a Chebyshev series.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, int N, the number of Chebyshev coefficients.
//
//    Output, double SFL::r8_CSEVL, the Chebyshev series evaluated at X.
//
{
  double b0;
  double b1;
...
double SFL::r8_dawson ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_DAWSON evaluates Dawson's integral of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_DAWSON, the value of Dawson's integral at X.
//
{
  ...
  {
    eps = SFL::r8_mach ( 3 );
    ntdaw  = SFL::r8_inits ( dawcs,  21, 0.1 * eps );
    ntdaw2 = SFL::r8_inits ( daw2cs, 45, 0.1 * eps );
    ntdawa = SFL::r8_inits ( dawacs, 75, 0.1 * eps );
    xsml = sqrt ( 1.5 * eps );
    xbig = sqrt ( 0.5 / eps );
    xmax = exp ( SFL::r8_min ( - log ( 2.0 * SFL::r8_mach ( 1 ) ), 
      log ( SFL::r8_mach ( 2 ) ) ) - 0.01 );
  }
  ...
  {
    value = x * ( 0.75 + SFL::r8_csevl ( 2.0 * y * y - 1.0, dawcs, ntdaw ) );
  }
  else if ( y <= 4.0 )
  {
    value = x * ( 0.25 + SFL::r8_csevl ( 0.125 * y * y - 1.0, daw2cs, ntdaw2 ) );
  }
  else if ( y < xbig )
  {
    value = ( 0.5 + SFL::r8_csevl ( 32.0 / y / y - 1.0, dawacs, ntdawa ) ) / x;
  }
  else if ( y <= xmax )
  {
    value = 0.5 / x;
...
double SFL::r8_e1 ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_E1 evaluates the exponential integral E1 for an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_E1, the exponential integral E1 evaluated at X.
//
{
  ...
  {
    eta = 0.1 * SFL::r8_mach ( 3 );
    ntae10 = SFL::r8_inits ( ae10cs, 50, eta );
    ntae11 = SFL::r8_inits ( ae11cs, 60, eta );
    ntae12 = SFL::r8_inits ( ae12cs, 41, eta );
    nte11 = SFL::r8_inits ( e11cs, 29, eta );
    nte12 = SFL::r8_inits ( e12cs, 25, eta );
    ntae13 = SFL::r8_inits ( ae13cs, 50, eta );
    ntae14 = SFL::r8_inits ( ae14cs, 64, eta );
    xmax = - log ( SFL::r8_mach ( 1 ) );
    xmax = xmax - log ( xmax );
  }
  ...
  {
    value = exp ( - x ) / x * ( 1.0 
      + SFL::r8_csevl ( 64.0 / x + 1.0, ae10cs, ntae10 ) );
  }
  else if ( x <= - 8.0 )
  {
    value = exp ( - x ) / x * ( 1.0 
      + SFL::r8_csevl ( ( 64.0 / x + 5.0 ) / 3.0, ae11cs, ntae11 ) );
  }
  else if ( x <= - 4.0 )
  {
    value = exp ( - x ) / x * (1.0 
      + SFL::r8_csevl ( 16.0 / x + 3.0, ae12cs, ntae12 ) );
  }
  else if ( x <= - 1.0 )
  {
    value = - log ( - x ) 
      + SFL::r8_csevl ( ( 2.0 * x + 5.0 ) / 3.0, e11cs, nte11 );
  }
  else if ( x == 0.0 )
  {
    cerr << "\n";
    cerr << "SFL::r8_E1 - Fatal error!\n";
    cerr << "  X is zero.\n";
    exit ( 1 );
  }
  else if ( x <= 1.0 )
  {
    value = ( - log ( abs ( x ) ) - 0.6875 + x ) 
      + SFL::r8_csevl ( x, e12cs, nte12 );
  }
  else if ( x <= 4.0 )
  {
    value = exp ( - x ) / x * ( 1.0 
      + SFL::r8_csevl ( ( 8.0 / x - 5.0 ) / 3.0, ae13cs, ntae13 ) );
  }
  else if ( x <= xmax )
  {
    value = exp ( - x ) / x * ( 1.0 
      + SFL::r8_csevl ( 8.0 / x - 1.0, ae14cs, ntae14 ) );
  }
  else
  {
    value = 0.0;
...
double SFL::r8_ei ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_EI evaluates the exponential integral Ei for an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_EI, the exponential integral Ei evaluated at X.
//
{
  double value;
...
double SFL::r8_erf ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_ERF evaluates the error function of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_ERF, the error function of X.
//
{
  ...
  if ( nterf == 0 )
  {
    nterf = SFL::r8_inits ( erfcs, 21, 0.1 * SFL::r8_mach ( 3 ) );
    xbig = sqrt ( - log ( sqrtpi * SFL::r8_mach ( 3 ) ) );
    sqeps = sqrt ( 2.0 * SFL::r8_mach ( 3 ) );
  }
  ...
  {
    value = x * ( 1.0 + SFL::r8_csevl ( 2.0 * x * x - 1.0, erfcs, nterf ) );
  }
  else if ( y <= xbig )
  {
    value = 1.0 - SFL::r8_erfc ( y );
    if ( x < 0.0 )
    {
      value = - value;
    }
...
double SFL::r8_erfc ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_ERFC evaluates the co-error function of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_ERFC, the co-error function of X.
//
{
  ...
  {
    eta = 0.1 * SFL::r8_mach ( 3 );
    nterf = SFL::r8_inits ( erfcs, 21, eta );
    nterfc = SFL::r8_inits ( erfccs, 59, eta );
    nterc2 = SFL::r8_inits ( erc2cs, 49, eta );

    xsml = - sqrt ( - log ( sqrtpi * SFL::r8_mach ( 3 ) ) );
    xmax = sqrt (- log ( sqrtpi * SFL::r8_mach ( 1 ) ) );
  ...
  {
    value = 1.0 - x * ( 1.0 
      + SFL::r8_csevl ( 2.0 * x * x - 1.0, erfcs, nterf ) );
    return value;
  }
  ...
  if ( y <= 4.0 )
  {
    value = exp ( - y ) / abs ( x ) * ( 0.5 
      + SFL::r8_csevl ( ( 8.0 / y - 5.0 ) / 3.0, erc2cs, nterc2 ) );
  }
  else 
  {
    value = exp ( - y ) / abs ( x ) * ( 0.5 
      + SFL::r8_csevl ( 8.0 / y - 1.0, erfccs, nterfc ) );
  }
...
double SFL::r8_exprel ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_EXPREL evaluates the exponential relative error term of an R8 argument.
//
//  Discussion:
//
//    The relative error term is ( exp ( x ) - 1 ) / x.
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_EXPREL, the exponential relative error term
//    at X.
//
{
  double absx;
...
double SFL::r8_fac ( int n )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_FAC evaluates the factorial of an I4 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, int N, the argument.
//
//    Output, double SFL::r8_FAC, the factorial of N.
//
{
  ...
  {
    x = ( double ) ( n + 1 );
    value = exp ( ( x - 0.5 ) * log ( x ) - x + sq2pil + SFL::r8_lgmc ( x ) );
  }
  else
  {
    cerr << "\n";
...
double SFL::r8_gami ( double a, double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_GAMI evaluates the incomplete gamma function for an R8 argument.
//
//  Discussion:
//
//    GAMI = Integral ( 0 <= T <= X ) exp ( - t ) * t^( a - 1 )  dt
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_GAMI, the value of the incomplete gamma function.
//
{
  double factor;
  double value;
  ...
  {
    factor = exp ( SFL::r8_lngam ( a ) + a * log ( x ) );
    value = factor * SFL::r8_gamit ( a, x );
  }
  return value;
}
//****************************************************************************80

double SFL::r8_gamic ( double a, double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_GAMIC evaluates the complementary incomplete gamma function.
//
//  Discussion:
//
//    GAMIC = integral ( x <= t < oo ) exp(-t) * t^(a-1) dt
...
//    Input, double X, the evaluation point.
//
//    Output, double SFL::r8_GAMIC, the value of the incomplete 
//    gamma function.
//
{
  double aeps;
  ...
  if ( eps == 0.0 )
  {
    eps = 0.5 * SFL::r8_mach ( 3 );
    sqeps = sqrt ( SFL::r8_mach ( 4 ) );
    alneps = - log ( SFL::r8_mach ( 3 ) );
    bot = log ( SFL::r8_mach ( 1 ) );
  }

  if ( x < 0.0 )
  {
    cerr << "\n";
    cerr << "SFL::r8_GAMIC - Fatal error!\n";
    cerr << "  X < 0.\n";
    exit ( 1 );
  }
  ...
  if ( x == 0.0 )
  {
    ...
    value = exp ( SFL::r8_lngam ( a + 1.0 ) - log ( a ) );
  ...
  if ( x < 1.0 )
  {
    if ( a <= 0.5 && abs ( aeps ) <= 0.001 )
    {
      ...
      if ( e * abs ( aeps ) <= eps )
      {
        value = SFL::r8_gmic ( a, x, alx );
        return value;
      }
    }

    SFL::r8_lgams ( a + 1.0, algap1, sgngam );
    gstar = SFL::r8_gmit ( a, x, algap1, sgngam, alx );
  ...
  else
  {
    if ( a < x )
    {
      value = exp ( SFL::r8_lgic ( a, x, alx ) );
      return value;
    }

    sgngam = 1.0;
    algap1 = SFL::r8_lngam ( a + 1.0 );
    sgngs = 1.0;
    alngs = SFL::r8_lgit ( a, x, algap1 );
  }
...
double SFL::r8_gamit ( double a, double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_GAMIT evaluates Tricomi's incomplete gamma function for an R8 argument.
//
//  Discussion:
//
//      GAMIT = x^(-a) / gamma(a) 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_GAMIT, the function value.
//
{
  double aeps;
  double ainta;
  ...
  if ( x <= 1.0 )
  {
    ...
    value = SFL::r8_gmit ( a, x, algap1, sgngam, alx );
    return value;
  }

  if ( x <= a )
  {
    t = SFL::r8_lgit (a, x, SFL::r8_lngam ( a + 1.0 ) );
    value = exp ( t );
    return value;
  }

  alng = SFL::r8_lgic ( a, x, alx );
//
//  Evaluate in terms of log (SFL::r8_gamic (a, x))
//
  h = 1.0;
...
void SFL::r8_gaml ( double &xmin, double &xmax )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_GAML evaluates bounds for an R8 argument of the gamma function.
//
//  Discussion:
//
//    This function calculates the minimum and maximum legal bounds 
  ...
  for ( i = 1; i <= 10; i++ )
  {
    ...
    if ( abs ( xmin - xold ) < 0.005 )
    {
      ...
      for ( j = 1; j <= 10; j++ )
      {
        ...
        {
          xmax = xmax - 0.01;
          xmin = SFL::r8_max ( xmin, - xmax + 1.0 );
          return;
        }
      }
      cerr << "\n";
      cerr << "SFL::r8_GAML - Fatal error!\n";
      cerr << "  Unable to find XMAX.\n";
      exit ( 1 );
    }
  }
...
double SFL::r8_gamma ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_GAMMA evaluates the gamma function of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_GAMMA, the gamma function of X.
//
{
  ...
  if ( ngcs == 0 )
  {
    ngcs = SFL::r8_inits ( gcs, 42, 0.1 * SFL::r8_mach ( 3 ) );
    SFL::r8_gaml ( xmin, xmax );
    xsml = exp ( SFL::r8_max ( log ( SFL::r8_mach ( 1 ) ),
      - log ( SFL::r8_mach ( 2 ) ) ) + 0.01 );
    dxrel = sqrt ( SFL::r8_mach ( 4 ) );
  }
  ...
  if ( y <= 10.0 )
  {
    ...
    y = x - ( double ) ( n );
    n = n - 1;
    value = 0.9375 + SFL::r8_csevl ( 2.0 * y - 1.0, gcs, ngcs );
    ...
    else if ( n < 0 )
    {
      ...
      if ( x < - 0.5 && abs ( ( x - SFL::r8_aint ( x - 0.5 ) ) / x ) < dxrel )
      {
        cerr << "\n";
        cerr << "SFL::r8_GAMMA - Warning!\n";
...
//
    ...
    value = exp ( ( y - 0.5 ) * log ( y ) - y + sq2pil + SFL::r8_lgmc ( y ) );
    ...
    if ( abs ( ( x - SFL::r8_aint ( x - 0.5 ) ) / x ) < dxrel )
    {
      cerr << "\n";
      cerr << "SFL::r8_GAMMA - Warning!\n";
      cerr << "  X too near a negative int,\n";
...
double SFL::r8_gamr ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_GAMR evaluates the reciprocal gamma function of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_GAMR, the value of the reciprocal gamma
//    function at X.
//
{
  double alngx;
  ...
  if ( x <= 0.0 && SFL::r8_aint ( x ) == x )
  {
    value = 0.0;
  } 
  else if ( abs ( x ) <= 10.0 )
  {
    value = 1.0 / SFL::r8_gamma ( x );
  }
  else
  {
    SFL::r8_lgams ( x, alngx, sgngx );
    value = sgngx * exp ( - alngx );
  }
  return value;
}
...
double SFL::r8_gmic ( double a, double x, double alx )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_GMIC: complementary incomplete gamma, small X, A near negative int.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, double SFL::r8_GMIC, the complementary incomplete 
//    gamma function.
//
{
  double alng;
  ...
  alng = log ( value ) - SFL::r8_lngam ( fm + 1.0 );
  ...
  {
    value = value
      + SFL::r8_sign ( s ) * exp ( - fm * alx + log ( abs ( s ) / fm ) );
  }
...
double SFL::r8_gmit ( double a, double x, double algap1, double sgngam, double alx )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_GMIT: Tricomi's incomplete gamma function for small X.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double ALX, the logarithm of X.
//
//    Output, double SFL::r8_GMIT, the Tricomi incomplete gamma function.
//
{
  double ae;
  double aeps;
  ...
  algs = - SFL::r8_lngam ( 1.0 + aeps ) + log ( s );
  s = 1.0;
  m = - ma - 1;
  t = 1.0;
...
int SFL::r8_inits ( double dos[], int nos, double eta )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_INITS initializes a Chebyshev series.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, int SFL::r8_INITS, the number of terms of the series needed
//    to ensure the requested accuracy.
//
{
  double err;
...
double SFL::r8_int ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_INT returns the integer part of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_INT, the integer part of X.
//
{
  int i;
  int ibase;
  ...
  if ( npart == 0 )
  {
    ibase = SFL::i4_mach ( 10 );
    xmax = 1.0 / SFL::r8_mach ( 4 );
    xbig = SFL::r8_min ( ( double ) ( SFL::i4_mach ( 9 ) ), 1.0 / SFL::r8_mach ( 4 ) );
    scale = ( double ) SFL::i4_pow ( ibase,
...
void SFL::r8_knus ( double xnu, double x, double &bknu, double &bknu1, int &iswtch )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_KNUS computes a sequence of K Bessel functions.
//
//  Discussion:
//
//    This routine computes Bessel functions 
...
//
{
  ...
  {
    eta = 0.1 * SFL::r8_mach ( 3 );
    ntc0k = SFL::r8_inits ( c0kcs, 29, eta );
    ntznu1 = SFL::r8_inits ( znu1cs, 20, eta );
    xnusml = sqrt ( SFL::r8_mach ( 3 ) / 8.0 );
    xsml = 0.1 * SFL::r8_mach ( 3 );
    alnsml = log ( SFL::r8_mach ( 1 ) );
    alnbig = log ( SFL::r8_mach ( 2 ) );
    alneps = log ( 0.1 * SFL::r8_mach ( 3 ) );
  }

  if ( xnu < 0.0 || 1.0 <= xnu )
  {
    cerr << "\n";
    cerr << "SFL::r8_KNUS - Fatal error!\n";
    cerr << "  XNU < 0 or 1 <= XNU.\n";
    exit ( 1 );
  }
...
//
    ...
    a0 = 0.5 * SFL::r8_gamma ( 1.0 + v );
    b0 = 0.5 * SFL::r8_gamma ( 1.0 - v );
    c0 = - euler;

    if ( 0.5 <= ztov && xnusml < v )
    {
      c0 = - 0.75 + SFL::r8_csevl ( ( 8.0 * v ) * v - 1.0, c0kcs, ntc0k );
    }
    ...
    else
    {
      alpha[0] = c0 - alnz * ( 0.75 +
        SFL::r8_csevl ( vlnz / 0.35 + 1.0, znu1cs, ntznu1 ) ) * b0;
    }
...
//
  else
  {
    ...
    an = - 0.60 - 1.02 / x;
    bn = - 0.27 - 0.53 / x;
    nterms = SFL::i4_min ( 32, SFL::i4_max ( 3, ( int ) ( an + bn * alneps ) ) );
    ...
    for ( inu = 1; inu <= 2; inu++ )
    {
      ...
      {
        xmu = 4.0 * ( abs ( xnu ) + 1.0 ) * ( abs ( xnu ) + 1.0 );
      }
...
double SFL::r8_lbeta ( double a, double b )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_LBETA evaluates the logarithm of the beta function of R8 arguments.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double A, B, the arguments.
//
//    Output, double SFL::r8_LBETA, the logarithm of the beta function of A
//    and B.
//
{
  double corr;
  ...
  {
    cerr << "\n";
    cerr << "SFL::r8_LBETA - Fatal error!\n";
    cerr << "  Both arguments must be greater than 0.\n";
    exit ( 1 );
  }
  else if ( p < 10.0 && q <= 10.0 )
  {
    value = log ( SFL::r8_gamma ( p ) * ( SFL::r8_gamma ( q ) / SFL::r8_gamma ( p + q ) ) );
  }
  else if ( p < 10.0 )
  {
    corr = SFL::r8_lgmc ( q ) - SFL::r8_lgmc ( p + q );

    value = SFL::r8_lngam ( p ) + corr + p - p * log ( p + q ) +
      ( q - 0.5 ) * SFL::r8_lnrel ( - p / ( p + q ) );
  }
  else
  {
    corr = SFL::r8_lgmc ( p ) + SFL::r8_lgmc ( q ) - SFL::r8_lgmc ( p + q );

    value = - 0.5 * log ( q ) + sq2pil + corr 
      + ( p - 0.5 ) * log ( p / ( p + q ) ) 
      + q * SFL::r8_lnrel ( - p / ( p + q ) );
  }
  return value;
}
...
void SFL::r8_lgams ( double x, double &algam, double &sgngam )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_LGAMS evaluates the log of |gamma(x)| and sign, for an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
{
  ...
  algam = SFL::r8_lngam ( x );
  sgngam = 1.0;

  if ( x <= 0.0 )
  {
    k = ( int ) ( fmod ( - SFL::r8_aint ( x ), 2.0 ) + 0.1 );
...
double SFL::r8_lgic ( double a, double x, double alx )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_LGIC evaluates the log complementary incomplete gamma function for large X.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double ALX, the logarithm of X.
//
//    Output, double SFL::r8_LGIC, the log complementary incomplete 
//    gamma function.
//
{
  static double eps = 0.0;
...
double SFL::r8_lgit ( double a, double x, double algap1 )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_LGIT evaluates the log of Tricomi's incomplete gamma function.
//
//  Discussion:
//
//    Perron's continued fraction is used for large X and X <= A.
...
//
//    Output, double SFL::r8_LGIT, the log of Tricomi's incomplete
//    gamma function.
//
{
  double a1x;
...
double SFL::r8_lgmc ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_LGMC evaluates the log gamma correction factor for an R8 argument.
//
//  Discussion:
//
//    For 10 <= X, compute the log gamma correction factor so that
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_LGMC, the correction factor.
//
{
  ...
  if ( nalgm == 0 )
  {
    nalgm = SFL::r8_inits ( algmcs, 15, SFL::r8_mach ( 3 ) );
    xbig = 1.0 / sqrt ( SFL::r8_mach ( 3 ) );
    xmax = exp ( SFL::r8_min ( log ( SFL::r8_mach ( 2 ) / 12.0 ), 
      - log ( 12.0 * SFL::r8_mach ( 1 ) ) ) );
  }

  if ( x < 10.0 )
  {
    cerr << "\n";
    cerr << "SFL::r8_LGMC - Fatal error!\n";
    cerr << "  X must be at least 10.\n";
    exit ( 1 );
  }
  else if ( x < xbig )
  {
    value = SFL::r8_csevl ( 2.0 * ( 10.0 / x ) 
...
double SFL::r8_li ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_LI evaluates the logarithmic integral for an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_LI, the logarithmic integral evaluated at X.
//
{
  static double sqeps = 0.0;
  double value;
  ...
  if ( sqeps == 0.0 )
  {
    sqeps = sqrt ( SFL::r8_mach ( 3 ) );
  }

  if ( x < 0.0 )
  {
    cerr << "\n";
    cerr << "SFL::r8_LI - Fatal error!\n";
    cerr << "  Function undefined for X <= 0.\n";
    exit ( 1 );
  }
  ...
  {
    cerr << "\n";
    cerr << "SFL::r8_LI - Warning!\n";
    cerr << "  Answer less than half precision.\n";
    cerr << "  X is too close to 1.\n";
  }

  value = SFL::r8_ei ( log ( x ) );

  return value;
}
//****************************************************************************80

double SFL::r8_lngam ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_LNGAM: log of the absolute value of gamma of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_LNGAM, the logarithm of the absolute value of
//    the gamma function of X.
//
{
  static double dxrel = 0.0;
  ...
  if ( xmax == 0.0 )
  {
    xmax = SFL::r8_mach ( 2 ) / log ( SFL::r8_mach ( 2 ) );
    dxrel = sqrt ( SFL::r8_mach ( 4 ) );
  }
  ...
  if ( 0.0 < x )
  {
    value = sq2pil + ( x - 0.5 ) * log ( x ) - x + SFL::r8_lgmc ( y );
    return value;
  }
  ...
  value = sqpi2l + ( x - 0.5 ) * log ( y ) - x - log ( sinpiy ) - SFL::r8_lgmc ( y );

  if ( abs ( ( x - SFL::r8_aint ( x - 0.5 ) ) * value / x ) < dxrel )
  {
    cerr << "\n";
    cerr << "SFL::r8_LNGAM - Warning!\n";
    cerr << "  Result is half precision because\n";
...
double SFL::r8_lnrel ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_LNREL evaluates log ( 1 + X ) for an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_LNREL, the value of LOG ( 1 + X ).
//
{
  ...
  if ( nlnrel == 0 )
  {
    nlnrel = SFL::r8_inits ( alnrcs, 43, 0.1 * SFL::r8_mach ( 3 ) );
    xmin = - 1.0 + sqrt ( SFL::r8_mach ( 4 ) );
  }

  if ( x <= - 1.0 )
  {
    cerr << "\n";
    cerr << "SFL::r8_LNREL - Fatal error!\n";
    cerr << "  X <= -1.\n";
    exit ( 1 );
  }
  else if ( x < xmin )
  ...
  if ( abs ( x ) <= 0.375 )
  {
    value = x * ( 1.0 - x * SFL::r8_csevl ( x / 0.375, alnrcs, nlnrel ) );
  }
  else
  {
    value = log ( 1.0 + x );
...
double SFL::r8_mach ( int i )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_MACH returns double precision real machine constants.
//
//  Discussion:
//
...
//    exponent, then
//
//      SFL::r8_MACH(1) = B^(EMIN-1), the smallest positive magnitude.
//      SFL::r8_MACH(2) = B^EMAX*(1-B^(-T)), the largest magnitude.
//      SFL::r8_MACH(3) = B^(-T), the smallest relative spacing.
//      SFL::r8_MACH(4) = B^(1-T), the largest relative spacing.
//      SFL::r8_MACH(5) = log10(B).
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    1 <= I <= 5.
//
//    Output, double SFL::r8_MACH, the value of the chosen parameter.
//
{
  double value;
...
void SFL::r8_machar ( long int *ibeta, long int *it, long int *irnd, long int *ngrd,
  long int *machep, long int *negep, long int *iexp, long int *minexp,
...
//  Purpose:
//
//    SFL::r8_MACHAR computes machine constants for R8 arithmetic.
//
//  Discussion:
//
//    This routine determines the parameters of the floating-point 
...
double SFL::r8_max ( double x, double y )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_MAX returns the maximum of two R8's.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//
//    Output, double SFL::r8_MAX, the maximum of X and Y.
//
{
  double value;
...
double SFL::r8_min ( double x, double y )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_MIN returns the minimum of two R8's.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license.
...
//    Input, double X, Y, the quantities to compare.
//
//    Output, double SFL::r8_MIN, the minimum of X and Y.
//
{
  double value;
...
double SFL::r8_mod ( double x, double y )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_MOD returns the remainder of R8 division.
//
//  Discussion:
//
//    If
...
//  Example:
//
//        X         Y     SFL::r8_MOD   SFL::r8_MOD  Factorization
//
//      107        50       7     107 =  2 *  50 + 7
//      107       -50       7     107 = -2 * -50 + 7
//     -107        50      -7    -107 = -2 *  50 - 7
...
//
//    Output, double SFL::r8_MOD, the remainder when X is divided by Y.
//
{
  double value;
  ...
  {
    cerr << "\n";
    cerr << "SFL::r8_MOD - Fatal error!\n";
    cerr << "  SFL::r8_MOD ( X, Y ) called with Y = " << y << "\n";
    exit ( 1 );
  }
...
double SFL::r8_mop ( int i )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_MOP returns the I-th power of -1 as an R8 value.
//
//  Discussion:
//
...
//    Input, int I, the power of -1.
//
//    Output, double SFL::r8_MOP, the I-th power of -1.
//
{
  double value;
...
double SFL::r8_pak ( double y, int n )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_PAK packs a base 2 exponent into an R8.
//
//  Discussion:
//
//    This routine is almost the inverse of SFL::r8_UPAK.  It is not exactly 
//    the inverse, because abs(x) need not be between 0.5 and 1.0.  
//    If both SFL::r8_PAK and 2.0^n were known to be in range, we could compute
//    SFL::r8_PAK = x * 2.0^n .
//
//  Licensing:
//
...
//
//    Output, double SFL::r8_PAK, the packed value.
//
{
  ...
  if ( nmin == 0 )
  {
    ...
    if ( SFL::i4_mach ( 10 ) != 2 );
    {
      aln2b = SFL::r8_mach ( 5 ) * aln210;
    }
    nmin = aln2b * ( double ) ( SFL::i4_mach ( 15 ) );
    nmax = aln2b * ( double ) ( SFL::i4_mach ( 16 ) );
  }

  SFL::r8_upak ( y, value, ny );
...
double SFL::r8_poch ( double a, double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_POCH evaluates Pochhammer's function of R8 arguments.
//
//  Discussion:
//
//    POCH ( A, X ) = Gamma ( A + X ) / Gamma ( A ).
...
//
//    Output, double SFL::r8_POCH, the Pochhammer function of A and X.
//
{
  double absa;
  double absax;
  ...
  if ( ax <= 0.0 && SFL::r8_aint ( ax ) == ax )
  {
    if ( 0.0 < a || SFL::r8_aint ( a ) != a )
    {
      cerr << "\n";
      cerr << "SFL::r8_POCH - Fatal error!\n";
...
//
    ...
    else if ( - 20.0 < SFL::r8_min ( a + x, a ) )
    {
      n = ( int ) ( x );
      ia = ( int ) ( a );
      value = SFL::r8_mop ( n ) * SFL::r8_fac ( - ia ) / SFL::r8_fac ( - ia - n );
    }
    else
    {
      n = ( int ) ( x );
      value = SFL::r8_mop ( n ) * exp ( ( a - 0.5 ) 
        * SFL::r8_lnrel ( x / ( a - 1.0 ) ) 
        + x * log ( - a + 1.0 - x ) - x 
        + SFL::r8_lgmc ( - a + 1.0 ) 
        - SFL::r8_lgmc ( - a - x + 1.0 ) );
    }
...
//
  ...
  if ( SFL::r8_max ( absax, absa ) <= 20.0 )
  {
    value = SFL::r8_gamma ( a + x ) * SFL::r8_gamr ( a );
    return value;
  }

  if ( 0.5 * absa < abs ( x ) )
  {
    SFL::r8_lgams ( a + x, alngax, sgngax );
    SFL::r8_lgams ( a, alnga, sgnga );
    value = sgngax * sgnga * exp ( alngax - alnga );
    return value;
  }
//
...
//
  ...
  value = exp ( ( b - 0.5 ) * SFL::r8_lnrel ( x / b ) 
    + x * log ( b + x ) - x + SFL::r8_lgmc ( b + x ) - SFL::r8_lgmc ( b ) );
...
double SFL::r8_poch1 ( double a, double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_POCH1 evaluates a quantity related to Pochhammer's symbol.
//
//  Discussion:
//
//    Evaluate a generalization of Pochhammer's symbol for special
...
//    Input, double X, the evaluation point.
//
//    Output, double SFL::r8_POCH1, the value of the function.
//
{
  double absa;
  double absx;
  ...
  if ( sqtbig == 0.0 )
  {
    sqtbig = 1.0 / sqrt ( 24.0 * SFL::r8_mach ( 1 ) );
    alneps = log ( SFL::r8_mach ( 3 ) );
  }
  ...
  if ( 0.1 * absa < absx || 0.1 < absx * log ( SFL::r8_max ( absa, 2.0 ) ) )
  {
    value = SFL::r8_poch ( a, x );
    value = ( value - 1.0 ) / x;
    return value;
  }
  ...
  poly1 = ( x - 1.0 ) * poly1;
  value = SFL::r8_exprel ( q ) * ( alnvar + q * poly1 ) + poly1;
//
//  we have SFL::r8_poch1(b,x), but bp is small, so we use backwards recursion
//  to obtain SFL::r8_poch1(bp,x).
//
  for ( ii = 1; ii <= incr; ii++ )
  {
    i = incr - ii;
...
//
//  we have SFL::r8_poch1(bp,x), but a is lt -0.5.  we therefore use a reflection
//  formula to obtain SFL::r8_poch1(a,x).
//
  sinpxx = sin ( pi * x ) / x;
  sinpx2 = sin ( 0.5 * pi * x );
  trig = sinpxx * SFL::r8_cot ( pi * b ) - 2.0 * sinpx2 * ( sinpx2 / x );
...
double SFL::r8_psi ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_PSI evaluates the psi function of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_PSI, the psi function of X.
//
{
  ...
  if ( ntpsi == 0 )
  {
    ntpsi = SFL::r8_inits ( psics, 42, 0.1 * SFL::r8_mach ( 3 ) );
    ntapsi = SFL::r8_inits ( apsics, 16, 0.1 * SFL::r8_mach ( 3 ) );
    xbig = 1.0 / sqrt ( SFL::r8_mach ( 3 ) );
    dxrel = sqrt ( SFL::r8_mach ( 4 ) );
  }
  ...
  if ( y < 10.0 )
  {
    ...
    y = x - ( double ) ( n );
    n = n - 1;
    value = SFL::r8_csevl ( 2.0 * y - 1.0, psics, ntpsi );
  ...
  else
  {
    if ( y < xbig )
    {
      aux = SFL::r8_csevl ( 8.0 / y / y - 1.0, apsics, ntapsi );
    }
    else
    {
      aux = 0.0;
...
//  Purpose:
//
//    SFL::r8_REN is a simple random number generator.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//  Parameters:
//
//    Output, double SFL::r8_REN, the random value.
//
{
  static int iy = 100001;
  double value;
...
double SFL::r8_shi ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_SHI evaluates the hyperbolic sine integral Shi of an R8 argument.
//
//  Discussion:
//
//    Shi ( x ) = Integral ( 0 <= t <= x ) sinh ( t ) dt / t
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_SHI, the hyperbolic sine integral 
//    Shi evaluated at X.
//
{
  double absx;
  ...
  if ( nshi == 0 )
  {
    nshi = SFL::r8_inits ( shics, 10, 0.1 * SFL::r8_mach ( 3 ) );
    xsml = sqrt ( SFL::r8_mach ( 3 ) );
  }
  ...
  {
    value = x * ( 1.0 
      + SFL::r8_csevl ( 128.0 * x * x / 9.0 - 1.0, shics, nshi ) );
  }
  else
  {
    value = 0.5 * ( SFL::r8_ei ( x ) + SFL::r8_e1 ( x ) );
...
double SFL::r8_si ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_SI evaluates the sine integral Si of an R8 argument.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_SI, the sine integral Si evaluated at X.
//
{
  double absx;
  double cosx;
  ...
  if ( nsi == 0 )
  {
    nsi = SFL::r8_inits ( sics, 18, 0.1 * SFL::r8_mach ( 3 ) );
    xsml = sqrt ( SFL::r8_mach ( 3 ) );
  }
  ...
  {
    value = x * ( 0.75 + SFL::r8_csevl ( ( x * x - 8.0 ) * 0.125, sics, nsi ) );
  }
  else
  {
    SFL::r8_sifg ( absx, f, g );
...
void SFL::r8_sifg ( double x, double &f, double &g )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_SIFG is a utility routine.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license. 
...
-0.0000000000000000000000000000000383 };
  ...
  if ( nf1 == 0 )
  {
    tol = 0.1 * SFL::r8_mach ( 3 );
    nf1 = SFL::r8_inits ( f1cs, 43, tol );
    nf2 = SFL::r8_inits ( f2cs, 99, tol );
    ng1 = SFL::r8_inits ( g1cs, 44, tol );
    ng2 = SFL::r8_inits ( g2cs, 44, tol );
    ng3 = SFL::r8_inits ( g3cs, 56, tol );
    xbig = sqrt ( 1.0 / SFL::r8_mach ( 3 ) );
    xmaxf = exp ( SFL::r8_min ( - log ( SFL::r8_mach ( 1 ) ), 
      log ( SFL::r8_mach ( 2 ) ) ) - 0.01 );
    xmaxg = 1.0 / sqrt ( SFL::r8_mach ( 1 ) );
    xbnd = sqrt ( 50.0 );
    xbndg = sqrt ( 200.0 );
  }
  ...
  {
    cerr << "\n";
    cerr << "SFL::r8_SIFG - Fatal error!\n";
    cerr << "  Approximation invalid for X < 4.\n";
    exit ( 1 );
  }
  else if ( x <= xbnd )
  {
    f = ( 1.0 + SFL::r8_csevl ( ( 1.0 / x / x - 0.04125 )
      / 0.02125, f1cs, nf1 ) ) / x;
    g = ( 1.0 + SFL::r8_csevl ( ( 1.0 / x / x - 0.04125 )
      / 0.02125, g1cs, ng1 ) ) / x / x;
  }
  else if ( x <= xbig )
  {
    f = ( 1.0 + SFL::r8_csevl ( 100. / x / x - 1.0, f2cs, nf2 ) ) / x;
    if ( x <= xbndg )
    {
      g = ( 1.0 + SFL::r8_csevl ( ( 10000.0 / x / x - 125.0 ) 
        / 75.0, g2cs, ng2 ) ) / x / x;
    }
    else
    {
      g = ( 1.0 + SFL::r8_csevl ( 400.0 / x / x - 1.0, g3cs, ng3 ) ) / x / x;
    }
  }
  else
  {
...
double SFL::r8_sign ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_SIGN returns the sign of an R8.
//
//  Licensing:
//
//    This code is distributed under the GNU LGPL license.
...
//
//    Output, double SFL::r8_SIGN, the sign of X.
//
{
  double value;
...
double SFL::r8_spence ( double x )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_SPENCE evaluates a form of Spence's function for an R8 argument.
//
//  Discussion:
//
//    This function evaluates a form of Spence's function defined by
...
//    Input, double X, the argument.
//
//    Output, double SFL::r8_SPENCE, Spence's function evaluated at X.
//
{
  double aln;
  static int nspenc = 0;
  ...
  if ( nspenc == 0 )
  {
    nspenc = SFL::r8_inits ( spencs, 38, 0.1 * SFL::r8_mach ( 3 ) );
    xbig = 1.0 / SFL::r8_mach ( 3 );
  }
  ...
  else if ( x <= - 1.0 )
  {
    ...
    value = - pi26 - 0.5 * aln * ( 2.0 
      * log ( - x ) - aln ) + ( 1.0 + SFL::r8_csevl (
      4.0 / ( 1.0 - x ) - 1.0, spencs, nspenc ) ) / ( 1.0 - x );
  }
  else if ( x <= 0.0 )
  {
    value = - 0.5 * log ( 1.0 - x ) 
      * log ( 1.0 - x ) - x * ( 1.0 + SFL::r8_csevl ( 
      4.0 * x / ( x - 1.0 ) - 1.0, spencs, nspenc ) ) / ( x - 1.0 );
  }
  else if ( x <= 0.5 )
  {
    value = x * ( 1.0 + SFL::r8_csevl ( 4.0 * x - 1.0, spencs, nspenc ) );
  }
  else if ( x < 1.0 )
  {
    value = pi26 - log ( x ) * log ( 1.0 - x )
      - ( 1.0 - x ) * ( 1.0 + SFL::r8_csevl ( 4.0 
  ...
  else if ( x <= 2.0 )
  {
    value = pi26 - 0.5 * log ( x ) 
      * log ( ( x - 1.0 ) * ( x - 1.0 ) / x )
      + ( x - 1.0 ) * ( 1.0 + SFL::r8_csevl ( 4.0 
      * ( x - 1.0 ) / x - 1.0, spencs, nspenc ) ) / x;
  }
  else if ( x < xbig )
  {
    value = 2.0 * pi26 - 0.5 * log ( x ) * log ( x )
      - ( 1.0 + SFL::r8_csevl ( 4.0 / x - 1.0, spencs, nspenc ) ) / x;
  }
  else
  {
    value = 2.0 * pi26 - 0.5 * log ( x ) * log ( x );
...
void SFL::r8_upak ( double x, double &y, int &n )

//****************************************************************************80
//
//  Purpose:
//
//    SFL::r8_UPAK unpacks an R8 into a mantissa and exponent.
//
//  Discussion:
//
//    This function unpacks a floating point number x so that
...
void SFL::r8_admp ( double x, double &ampl, double &phi )
