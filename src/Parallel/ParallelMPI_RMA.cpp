...
Parallel::Parallel(Setup *setup) 
{
  ...
  Coord.resize(Range(DIR_X,DIR_SIZE-1)); Coord(Range(DIR_X, DIR_S)) = 0; Coord(Range(DIR_S+1, DIR_SIZE-1)) = -1;
...
#pragma omp parallel 
  ...
  else for(int dir=DIR_X; dir < decomp.size(); dir++)  decomposition(dir) = atoi(decomp[dir].c_str());
  ...
  checkValidDecomposition(setup, decomposition);
   ...
   int test_rank;
   MPI_Group_translate_ranks(CommV_group, 1, &dirMaster[DIR_V],  CommALL_group, &test_rank);
    std::cout << "222 MyRank : " << myRank << " myPos : " << Coord(DIR_V) << "  : " << dirMaster[DIR_V] << "  rank(ALL Root) : " << test_rank << std::endl;
    */
...
int Parallel::updateNeighbours(Array6d  Sendu, Array6d  Sendl, Array6d  Recvu, Array6d  Recvl, int dir)
{
#ifdef HELIOS_PARALLEL_MPI
     
        MPI_Irecv(Recvl.data(), Recvl.numElements(), MPI_DOUBLE, Talk(dir).rank_l, Talk(dir).psf_msg_tag[0], Comm[DIR_ALL], &Talk(dir).psf_msg_request[1]);
        MPI_Isend(Sendu.data(), Sendu.numElements(), MPI_DOUBLE, Talk(dir).rank_u, Talk(dir).psf_msg_tag[0], Comm[DIR_ALL], &Talk(dir).psf_msg_request[0]);
        if(Talk(dir).rank_u == MPI_PROC_NULL)   Recvl = 0.e0;
     
        MPI_Irecv(Recvu.data(), Recvu.numElements(), MPI_DOUBLE, Talk(dir).rank_u, Talk(dir).psf_msg_tag[1], Comm[DIR_ALL], &Talk(dir).psf_msg_request[3]); 
        MPI_Isend(Sendl.data(), Sendl.numElements(), MPI_DOUBLE, Talk(dir).rank_l, Talk(dir).psf_msg_tag[1], Comm[DIR_ALL], &Talk(dir).psf_msg_request[2]);
...
int Parallel::updateNeighbours(Array6d  SendXl, Array6d  SendXu, Array6d  SendYl, Array6d  SendYu, Array6d SendZl, Array6d SendZu, 
                               Array6d  RecvXl, Array6d  RecvXu, Array6d  RecvYl, Array6d  RecvYu, Array6d RecvZl, Array6d RecvZu) 
{
#ifdef HELIOS_PARALLEL_MPI 
      ...
      // For X-Direction 
      MPI_Irecv(RecvXl.data(), RecvXl.numElements(), MPI_DOUBLE, Talk(DIR_X).rank_l, Talk(DIR_X).phi_msg_tag[0], Comm[DIR_ALL], &msg_request[5]); 
      MPI_Isend(SendXu.data(), SendXu.numElements(), MPI_DOUBLE, Talk(DIR_X).rank_u, Talk(DIR_X).phi_msg_tag[0], Comm[DIR_ALL], &msg_request[4]);
      
      MPI_Irecv(RecvXu.data(), RecvXu.numElements(), MPI_DOUBLE, Talk(DIR_X).rank_u, Talk(DIR_X).phi_msg_tag[1], Comm[DIR_ALL], &msg_request[7]); 
      MPI_Isend(SendXl.data(), SendXl.numElements(), MPI_DOUBLE, Talk(DIR_X).rank_l, Talk(DIR_X).phi_msg_tag[1], Comm[DIR_ALL], &msg_request[6]);

      // For Y-Direction
      MPI_Irecv(RecvYl.data(), RecvYl.numElements(), MPI_DOUBLE, Talk(DIR_Y).rank_l, Talk(DIR_Y).phi_msg_tag[0], Comm[DIR_ALL], &msg_request[1]); 
      MPI_Isend(SendYu.data(), SendYu.numElements(), MPI_DOUBLE, Talk(DIR_Y).rank_u, Talk(DIR_Y).phi_msg_tag[0], Comm[DIR_ALL], &msg_request[0]);
      
      MPI_Irecv(RecvYu.data(), RecvYu.numElements(), MPI_DOUBLE, Talk(DIR_Y).rank_u, Talk(DIR_Y).phi_msg_tag[1], Comm[DIR_ALL], &msg_request[3]); 
      MPI_Isend(SendYl.data(), SendYl.numElements(), MPI_DOUBLE, Talk(DIR_Y).rank_l, Talk(DIR_Y).phi_msg_tag[1], Comm[DIR_ALL], &msg_request[2]);
      
      // For Z-Direction
      MPI_Irecv(RecvZl.data(), RecvZl.numElements(), MPI_DOUBLE, Talk(DIR_Z).rank_l, Talk(DIR_Z).phi_msg_tag[0], Comm[DIR_ALL], &msg_request[9]); 
      MPI_Isend(SendZu.data(), SendZu.numElements(), MPI_DOUBLE, Talk(DIR_Z).rank_u, Talk(DIR_Z).phi_msg_tag[0], Comm[DIR_ALL], &msg_request[8]);
      
      MPI_Irecv(RecvZu.data(), RecvZu.numElements(), MPI_DOUBLE, Talk(DIR_Z).rank_u, Talk(DIR_Z).phi_msg_tag[1], Comm[DIR_ALL], &msg_request[11]); 
      MPI_Isend(SendZl.data(), SendZl.numElements(), MPI_DOUBLE, Talk(DIR_Z).rank_l, Talk(DIR_Z).phi_msg_tag[1], Comm[DIR_ALL], &msg_request[10]);
...
MPI_Op Parallel::getMPIOp(int op) {
    switch(op) {
        case(OP_SUM) : return MPI_SUM; break;
        case(OP_MAX) : return MPI_MAX; break;
        case(OP_MIN) : return MPI_MIN; break;
...
MPI_Datatype Parallel::getMPIDataType(const std::type_info &T) {
    if     (T == typeid(double)) return MPI_DOUBLE;
    else if(T == typeid(int))    return MPI_INT;
    else if(T == typeid(std::complex<double>)) return MPI_DOUBLE;
...
double Parallel::collect(double dValue, int op, int dir, bool allreduce) {
#ifdef HELIOS_PARALLEL_MPI
        double global_dValue;
       // we need allreduce instead of reduce because H5TB need all process to have the same value
       check(MPI_Allreduce(&dValue, &global_dValue, 1, MPI_DOUBLE, getMPIOp(op), Comm[dir]), DMESG("MPI_Reduce")); 
...
bool Parallel::checkValidDecomposition(Setup *setup, Array1i decomposition) {
   ...
   // Simple Check if reasonable values are provided for decomposition
