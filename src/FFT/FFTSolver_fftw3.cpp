...
FFTSolver_fftw3::FFTSolver_fftw3(Setup *setup, Parallel *parallel, Geometry<HELIOS_GEOMETRY> *geo) : FFTSolver(setup, parallel, geo, Nx*Nky*Nz, Nx*(2*Nky-2), Nx, 2*Nky-2) {
   ...
   if(parallel->Coord(DIR_FFT) == 0) {
      ...
      perf_flag = (setup->get("FFTW3.Measure", 1) == 1) ? FFTW_MEASURE :  FFTW_ESTIMATE; 
      ...
      if(flags & FFT_X) {
         ...
         //data_X_kIn_rOut = (cmplxd *) fftw_alloc_complex(X_numElements*NyLD*NzLD*plasma->nfields);
         //data_X_rIn_kOut = (cmplxd *) fftw_alloc_complex(X_numElements*NyLD*NzLD*plasma->nfields);
      ...
      GeneralArrayStorage<4> storage_rX; storage_rX.ordering() = fourthDim, thirdDim,  secondDim, firstDim; storage_rX.base() = NxLlD , NkyLlD, NzLlD, 1;
         //GeneralArrayStorage<4> storage_rX; storage_rX.ordering() = fourthDim, thirdDim,  firstDim, secondDim; storage_rX.base() = NxLlD , NkyLlD, NzLlD, 1;
 ...
 //        GeneralArrayStorage<4> storage_cX; storage_cX.ordering() = fourthDim, thirdDim,  firstDim, secondDim; storage_cX.base() = X_NkxLlD, NkyLlD, NzLlD, 1;
      ...
      if(flags & FFT_Y & FFT_AA) {


  std::cout << " HEEEERE : *****************************"; 


          check(-1, DMESG("Don't use non AA"));     

                data_Y_kIn  = (cmplxd *) fftw_alloc_complex(2*NyLD*NxLD*NzLD*plasma->nfields);
                data_Y_kOut = (cmplxd *) fftw_alloc_complex(2*NyLD*NxLD*NzLD*plasma->nfields);
                ...
                int NkyLD_AA = 2*Nky;
                RkyLD_AA.setRange(NkyLuD-1, NkyLD_AA-1);
                ...
                GeneralArrayStorage<4> storage_cY; storage_cY.ordering() = fourthDim, thirdDim,  firstDim, secondDim; storage_cY.base() = NxLlD, NyLlD, NzLlD, 1;
                Array4z kYOut_t  (data_Y_kOut, shape(NxLD, NkyLD_AA, NzLD, plasma->nfields), neverDeleteData, storage_cY); kYOut.reference(kYOut_t ) ; kYOut = 0.; 
                Array4z kYIn_t   (data_Y_kIn , shape(NxLD, NkyLD_AA, NzLD, plasma->nfields), neverDeleteData, storage_cY); kYIn .reference(kYIn_t  ) ; kYIn  = 0.;
                   
                //                                                  howmany                                  stride distance, 
                plan_YForward  = fftw_plan_many_dft_r2c(1, &NyLD, NxLD*NzLD,                 rYIn.data(), NULL, 1, NyLD ,  (fftw_complex*) kYOut.data(), NULL, 1, NkyLD_AA, perf_flag);
                plan_YBackward = fftw_plan_many_dft_c2r(1, &NyLD, NxLD*NzLD, (fftw_complex*) kYIn.data(), NULL, 1, NkyLD_AA,                  rYOut.data(), NULL, 1, NyLD , perf_flag);
      ...
      if(flags & FFT_Y) {
                // array is too big (but for safety)
                data_Y_kIn  = (cmplxd *) fftw_alloc_complex(2*(NyLD/2+1)*NxLD*NzLD*plasma->nfields);
                data_Y_kOut = (cmplxd *) fftw_alloc_complex(2*(NyLD/2+1)*NxLD*NzLD*plasma->nfields);
                data_Y_rOut = (double *) fftw_alloc_real   (2*NyLD*NxLD*NzLD*plasma->nfields);
                data_Y_rIn  = (double *) fftw_alloc_real   (2*NyLD*NxLD*NzLD*plasma->nfields);
                ...
                //                                                  howmany                                  stride distance,
                // Take care of length of transform                                                
...
int FFTSolver_fftw3::solve(const int FFTtype, const int direction, const int N) {
        ...
        if     (FFTtype & FFT_XYZ & flags) check(-1, DMESG("No such FFT direction (did you set : FFTSolver.3D = 1 )"));
             ...
             if     (direction == FFT_FORWARD )  fftw_execute(plan_YForward);
                    ...
                    // Anti-Aliasing, set ky mode above Nyquist to zero
               //     kYIn   (RxLD, RkyLD_AA, RzLD, RFields) = cmplxd(0.,0.);
                    fftw_execute(plan_YBackward);
             }
             else   check(-1, DMESG("No such FFT direction"));
        }
...
FFTSolver_fftw3::~FFTSolver_fftw3() {
    ...
    //fftw_free(data_X_kIn_rOut);
    //fftw_free(data_X_rIn_kOut);
...
{
   Array3d AA_xyz(RxLD, RyLD, RzLD);

   kYIn = 0.;
   for(int y_k=NkyLlD; y_k<= NkyLuD;y_k++) kYIn(RxLD, y_k, RzLD, 1) = A(RxLD, y_k, RzLD); 
   solve(FFT_Y, FFT_BACKWARD, NxLD * NzLD);
   AA_xyz(RxLD, RyLD, RzLD) = rYOut(RxLD, RyLD, RzLD, 1);


   kYIn = 0.;
   //#-pragma omp parallel for
   for(int y_k=NkyLlD; y_k<= NkyLuD;y_k++) kYIn(RxLD, y_k, RzLD, 1) = B(RxLD, y_k, RzLD);
   solve(FFT_Y, FFT_BACKWARD, NxLD * NzLD);

   // multiply-values 
   for(int z=NzLlD; z<=NzLuD;z++) { for(int y=NyLlD; y<=NyLuD;y++) { for(int x= NxLlD; x <= NxLuD; x++) {
    AA_xyz(x,y,z) = AA_xyz(x,y,z) * rYOut(x,y,z,1) / (Norm_Y * Norm_Y);
   }}}
   
   rYIn(RxLD, RyLD, RzLD, 1) = AA_xyz(RxLD, RyLD, RzLD); 
