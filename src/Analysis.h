...
Geometry<HELIOS_GEOMETRY> *geo;
  ...
  template<typename tV, typename tM> cmplxd  F (int  x, int y, int z, tV v, tM m, int s) {
    //if (setup->VlasovType == VLASOV_LOCAL)     return pow2(plasma->species(s).m) * geometry->J(x,y,z) * plasma->B0 * sum(vlasov->f(x,y,z,v,m,s) + vlasov->f0(x,y,z,v,m,s));
    //else                                       return pow2(plasma->species(s).m) * geometry->J(x,y,z) * plasma->B0 * sum(vlasov->f(x,y,z,v,m,s));
  ...
  template<typename tV, typename tM> cmplxd  dF(int  x, int y, int z, tV v, tM m, int s) {
     ...
     *
     *  @out   double Array of Power density spectra in direction dir
     *
     */
     ...
     *  \f]
     *
     *  Normalized : Note that d6Z = m\hat{B} d\mu dv_\parallel
     *
     * @param species particle species \f[ \sigma \f]
     *
     * @out   the total energy of species
     */
     ...
     *
     * @in
     * const int a parallel velocity moment
     * const int b perpendicular velocity moment
     * const int s    species
     *
     */
    ...
    /*!
     *
     *  Get electro-static field energy, note that the field energy consist of various contributions
     *  depending on what kind of simualtions are performed. This is
     *
     *  \f[
