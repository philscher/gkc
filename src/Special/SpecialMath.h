   ...
   **/
   static inline double BesselI1( double x ) 
   {
      ...
      if (abs(x) <  3.75)  { 
        ...
        const double  p[] = { 0.5, 0.87890594, 0.51498869, 0.15084934, 0.2658733e-1, 0.301532e-2, 0.32411e-3 };
        const double y=pow2((x/3.75));
        I1=x*(p[0]+y*(p[1]+y*(p[2]+y*(p[3]+y*(p[4]+y*(p[5]+y*p[6]))))));
      
      } else {
        
        const double  q[] = { 0.39894228e0, -0.3988024e-1, -0.362018e-2, 0.163801e-2, -0.1031555e-1, 
                              0.2282967e-1, -0.2895312e-1, 0.1787654e-1, -0.420059e-2 } ;
        const double ax=abs(x);
        const double y=3.75/ax;
        
        I1=(exp(ax)/sqrt(ax))*(q[0]+y*(q[1]+y*(q[2]+y*(q[3]+y*(q[4]+y*(q[5]+y*(q[6]+y*(q[7]+y*q[8]))))))));
