...
VlasovCilk::VlasovCilk(Grid *_grid, Parallel *_parallel, Setup *_setup, FileIO *fileIO, Geometry<HELIOS_GEOMETRY> *_geo, FFTSolver *fft)    : Vlasov_LenardBernstein(_grid, _parallel, _setup, fileIO, _geo, fft),
    dphi_dx(FortranArray<3>()),   dphi_dy(FortranArray<3>()),   
    dAp_dx(FortranArray<3>()),    dAp_dy(FortranArray<3>()),    k2p_phi(FortranArray<3>()), nonLinearTerms(HeliosStorage4)
{
...
int VlasovCilk::solve(std::string equation_type, Fields *fields, Array6z _fs, Array6z _fss, double dt, int rk_step, int user_boundary_type) 
{
  ...
  if((equation_type == "2D_ES")  && (plasma->nfields == 1)) Vlasov_2D((A6z) _fs.dataZero(), (A6z) _fss.dataZero(), (A6z) f0.dataZero(), (A6z) f.dataZero(), (A6z) ft.dataZero(), (A5z) fields->phi.dataZero(), (A3z) k2p_phi.dataZero(), (A4z) nonLinearTerms.dataZero(), X.dataZero(), V.dataZero(), M.dataZero(), fields, dt, rk_step, _fs);
  else if((equation_type == "2D_EM")) Vlasov_EM((A6z) _fs.dataZero(), (A6z) _fss.dataZero(), (A6z) f0.dataZero(), (A6z) f.dataZero(), (A6z) ft.dataZero(), (A5z) fields->phi.dataZero(), (A3z) k2p_phi.dataZero(), (A3z) dphi_dx.dataZero(), (A4z) Xi.dataZero(), (A4z) G.dataZero(), X.dataZero(), V.dataZero(), M.dataZero(), fields, dt, rk_step);
...
Array4z VlasovCilk::calculatePhiNonLinearity(Array5z phi, Array6z f1, const int m, const int s) 
{
      ...
      for(int m=NmLlD; m<= NmLuD;m++) { 
       ...
       for(int z=NzLlD; z<= NzLuD;z++) { 
        ...
        for(int y_k=NkyLlD; y_k<= NkyLuD;y_k++) { for(int x=NxLlD; x<= NxLuD;x++) { 
             ...
             const cmplxd dky = cmplxd(0.,fft->ky(y_k));


             updateCFL(dphi_dx, dky*phi_, 0.);

                    
            const double shear = geo->getShear(x);
        ...
        cmplxd dg_dt = 
             ...
             dky* (-(w_n + w_T * (((V[v]*V[v])+ M[m])/Temp  - sub)) * F0 * phi_
      ...
      for(int m=NmLlD; m<= NmLuD;m++) { 
       ...
       for(int z=NzLlD; z<= NzLuD;z++) { 
        ...
        for(int y_k=NkyLlD; y_k<= NkyLuD;y_k++) { for(int x=NxLlD; x<= NxLuD;x++) { 
             ...
             Xi_max(DIR_Y) = max(Xi_max(DIR_Y), abs(dky*phi[s][m][z][y_k][x])); 
             Xi_max(DIR_Z) = max(Xi_max(DIR_Z), abs(dky*geo->getShear(x)));// * abs(phi[s][m][z][y_k][x])); 
...
{ 
   ...
   for(int s = NsLlD; s <= NsLuD; s++) {
      ...
      for(int m=NmLlD; m<= NmLuD;m++) { 
         ...
         // gyro-fluid model
       if(fields->gyroAverageModel == "Gyro-1") k2p_phi(RxLD, RkyLD, RzLD) = fields->gyroAverage(fields->Field0(RxLD, RkyLD, RzLD, Field::phi), 2, s,  Field::phi, true);
         //if(nonLinear)   calculatePhiNonLinearity(Xi, _fs,m, s);
         ...
         for(int y_k=NkyLlD; y_k<= NkyLuD;y_k++) { for(int x=NxLlD; x<= NxLuD;x++) { 
            ...
            Xi_max(DIR_X) = max(Xi_max(DIR_X), abs(dphi_dx[z][y_k][x]));
            Xi_max(DIR_Y) = max(Xi_max(DIR_Y), abs(dky*phi[s][m][z][y_k][x])); 
            Xi_max(DIR_Z) = max(Xi_max(DIR_Z), abs(dky*geo->getShear(x)* X[x]));// * abs(phi[s][m][z][y_k][x])); 
   ...
   cmplxd dg_dt = 
      ...
      // driving term (use dphi_dy instead of dXi_dy, because v * A does not vanish due to numerical errors)
      dky* (-(w_n + w_T * (((V[v]*V[v])+ M[m])/Temp  - sub)) * F0 * Xi_
             
      // add first order gyro-average term (zero when full-gyro)
            + 0.5 * w_T  * k2_phi[z][y_k][x] * F0
      // Landau Damping term and parallel ... ? - alpha  * V(v)* geo->getShear(x)  * ( g + sigma * phi * F0)) 
            - alpha  * V[v]* geo->getShear(x)  * ( g + sigma * Xi_ * F0 ))   
      // Collisional terms 
      + collisionBeta * (g  + V[v] * dfs_dv + v2_rms * ddfs_dvv)    ;
...
void VlasovCilk::setupXiAndG(
                           ...
                           cmplxd f0       [NsLD][NmLD][NzLB][NkyLD][NxLB  ][NvLB],
                           cmplxd phi       [NsLD][NmLD][NzLB][NkyLD][NxLB  ][NvLB],
                           cmplxd Ap       [NsLD][NmLD][NzLB][NkyLD][NxLB  ][NvLB],
                           cmplxd Bp       [NsLD][NmLD][NzLB][NkyLD][NxLB  ][NvLB],
                           cmplxd Xi       [NzLB][NkyLD][NxLB  ][NvLB],
                           cmplxd G        [NzLB][NkyLD][NxLB  ][NvLB],
  ...
  const int    N   = plasma->nfields;

  for(int z=NzLlB; z<= NzLuB;z++) { for(int y_k=NkyLlD; y_k<= NkyLuD;y_k++) { 
  for(int x=NxLlB; x<= NxLuB;x++) { for(int v=NvLlB  ; v<= NvLuB  ;v++ ) { 

     Xi[z][y_k][x][v] = phi[s][m][z][y_k][x][v] - ((N >= 2) ? aeb*V[v]*Ap[s][m][z][y_k][x][v] : 0.) - ((N >= 3) ? aeb*M[m]*Bp[s][m][z][y_k][x][v] : 0.);
