...
if sys.argv[1] == "Exe" : 
     ...
     decomposition = "1:2:1:1:1:1"
     helios_options = []
     
     for theta in [ 0.1]:
      for mass in [ 1.e-5, 5.e-5, 1./1000., 1./400.]:
        helios_options.append("\"DataOutput.OutputFileName=ConstantTheta_%.2f_Mass_%.5f_.h5;Geometry.Theta=%.2f\;Plasma.Species2.Mass=%.2e;\"" % (theta, mass, theta, mass))
     executeThin(sys.argv[2], helios_options, decomposition, queue='vh10140', system_shared=True)
...
if sys.argv[1] == "Ana":
    ...
    for fileh_name in sys.argv[4:]:
          try:
            ...
            result_A.append((gamma, 0.1))#fileh5.root.Geometry._v_attrs.Theta[0]))
            ...
            Setup['theta'] = 0.1#fileh5.root.Geometry._v_attrs.Theta[0]
