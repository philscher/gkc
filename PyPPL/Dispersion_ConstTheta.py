...
def getGrowth(ky_list, Setup, disp="Gyro", init = (-0.02 -.006j, 'halley')):
  mp.dps=15
  ...
  PlasmaDispersion = PyPPL.PlasmaDispersion
  ...
  for ky in ky_list:
    ...
    b   = ko2 * rho_te2
    G0  = PyPPL.Gamma0(b)
    G0mG1 = PyPPL.Gamma0(b) - PyPPL.Gamma1(b)
    ...
    if disp == 'Gyro-1':
      ...
      def DispersionRelation(w):
        ...
        Lambda = lambda_D2 * b + b
        zeta = w / kp
        Z = PlasmaDispersion(zeta) 
        # Take care of extra pie due to normalization
        return  - (1. - eta/2. * (1.))*kyp * Z \
                - eta * kyp * (zeta + zeta**2 * Z) + zeta * Z + 1. +  Lambda 


    elif disp == 'Gyro1stKin':
      def DispersionRelation(w):
        ...
        # proton
        sum1MG0 = lambda_D2 * b + mp.exp(b) * (1. - G0) +  mp.exp(b/m_ie) * (1. - G0/m_ie) 
    ...
    try:
      omega = complex(mp.findroot(DispersionRelation, init[0], solver=init[1], maxsteps=300))
    except : 
      omega = float('nan') + float('nan') * .1j
    print "ky : ", ky , " w : ", omega 
